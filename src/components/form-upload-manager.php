...
use awsmug\Lantern_Forms\DB_Objects\Taxonomy_Manager;
...
class Form_Upload_Manager extends Service {
	...
	public function upload_file( $file_id, $submission, $form, $element_id, $field = '_main', $allowed_mimes = null, $allowed_filesize = null ) {
		if ( ! isset( $_FILES[ $file_id ] ) ) {
			return new WP_Error( 'missing_file', __( 'No file was provided to upload.', 'lantern-forms' ) );
		}
		...
		$attachment_data = array(
			...
			'post_title' => sprintf( __( 'Form upload for submission #%1$s (form &#8220;%2$s&#8221;)', 'lantern-forms' ), $submission->id, $form->title ),
			'meta_input' => array(
		...
		if ( $allowed_filesize ) {
			...
			if ( (int) $filesize > (int) $allowed_filesize ) {
				return new WP_Error( 'upload_error', __( 'The file exceeds the maximum allowed size.', 'lantern-forms' ) );
			}
		}
		...
		if ( is_wp_error( $attachment_id ) ) {
			...
			if ( $allowed_mimes && 'upload_error' === $attachment_id->get_error_code() && __( 'Sorry, this file type is not permitted for security reasons.' ) === $attachment_id->get_error_message() ) { // @codingStandardsIgnoreLine
				return new WP_Error( 'upload_error', __( 'The file type is not permitted.', 'lantern-forms' ) );
			}
		...
		if ( ! $attachment_id ) {
			return new WP_Error( 'upload_error', __( 'The file could not be registered with the database.', 'lantern-forms' ) );
		}
	...
	public function delete_old_files( $submission, $form, $element_id, $field = '_main', $ignore_id = 0 ) {
		...
		foreach ( $attachment_ids as $attachment_id ) {
			...
			if ( ! $post ) {
				$result[] = new WP_Error( 'delete_error', __( 'The file could not be deleted.', 'lantern-forms' ) );
			} else {
				$result[] = $post->ID;
			}
		}
