...
abstract class Submission_Export {
	...
	protected function get_submission_columns( $form ) {
		$submission_columns = array(
			...
			'user' => array(
				...
				'callback' => function( $submission ) {
					if ( ! $submission->user_id ) {
						return __( 'not available', 'lantern-forms' );
					}
			...
			'date' => array(
				'label'    => __( 'Date', 'lantern-forms' ),
				'callback' => function( $submission ) {
					/* translators: 1: date format string, 2: time format string */
					$format = sprintf( _x( '%1$s %2$s', 'concatenating date and time format', 'lantern-forms' ), get_option( 'date_format' ), get_option( 'time_format' ) );
