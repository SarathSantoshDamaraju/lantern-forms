...
namespace awsmug\Lantern_Forms\Components;
...
class Submission_Export_Handler extends Service {
	...
	public function export_submissions( $mode, $form, $args = array() ) {
		if ( ! isset( $this->modes[ $mode ] ) ) {
			wp_die( esc_html__( 'Invalid submission export handler.', 'lantern-forms' ) );
		}
	...
	public function handle_export_action() {
		...
		if ( empty( $nonce ) ) {
			wp_die( esc_html__( 'Missing nonce.', 'lantern-forms' ) );
		}

		if ( ! wp_verify_nonce( $nonce, $this->nonce_action ) ) {
			wp_die( esc_html__( 'Invalid nonce.', 'lantern-forms' ) );
		}

		if ( empty( $form_id ) ) {
			wp_die( esc_html__( 'Missing form ID.', 'lantern-forms' ) );
		}

		$capabilities = $this->submission_manager->capabilities();
		if ( ! $capabilities || ! $capabilities->user_can_read() ) {
			wp_die( esc_html__( 'Insufficient permissions.', 'lantern-forms' ) );
		}

		$form = $this->submission_manager->get_parent_manager( 'forms' )->get( $form_id );
		if ( ! $form ) {
			wp_die( esc_html__( 'Invalid form ID.', 'lantern-forms' ) );
		}

		if ( empty( $mode ) ) {
			wp_die( esc_html__( 'Missing submission export handler.', 'lantern-forms' ) );
		}
	...
	public function render_export_form() {
		...
		?>
		<form class="lantern-export-form" action="<?php echo esc_url( admin_url( 'admin.php' ) ); ?>" method="POST">
			...
			<h3><?php esc_html_e( 'Export Submissions', 'lantern-forms' ); ?></h3>

			<p class="description">
				<?php esc_html_e( 'Here you can export all completed submissions in a file format of your choice.', 'lantern-forms' ); ?>
			</p>

			<label for="lantern-export-mode"><?php esc_html_e( 'Export as', 'lantern-forms' ); ?></label>
			<select id="lantern-export-mode" name="mode" style="margin-right:15px;">
				<?php foreach ( $this->modes as $slug => $mode ) : ?>
					<option value="<?php echo esc_attr( $slug ); ?>"><?php echo esc_html( $mode->get_title() ); ?></option>
				<?php endforeach; ?>
			</select>

			<label for="lantern-export-orderby"><?php esc_html_e( 'Order by', 'lantern-forms' ); ?></label>
			<select id="lantern-export-orderby" name="orderby">
				<option value="id"><?php esc_html_e( 'ID', 'lantern-forms' ); ?></option>
				<option value="timestamp"><?php esc_html_e( 'Date', 'lantern-forms' ); ?></option>
			</select>

			<label for="lantern-export-order" class="screen-reader-text"><?php esc_html_e( 'Order', 'lantern-forms' ); ?></label>
			<select id="lantern-export-order" name="order" style="margin-right:15px;">
				<option value="ASC"><?php esc_html_e( 'Ascending', 'lantern-forms' ); ?></option>
				<option value="DESC"><?php esc_html_e( 'Descending', 'lantern-forms' ); ?></option>
			</select>

			<label for="lantern-export-onechoice-columns"><?php esc_html_e( 'One Choice items', 'lantern-forms' ); ?></label>
			<select id="lantern-export-onechoice-columns" name="onechoice_columns" style="margin-right:15px;">
				<option value="one"><?php esc_html_e( 'Export all values in one column', 'lantern-forms' ); ?></option>
				<option value="multi"><?php esc_html_e( 'Export each value in a different column', 'lantern-forms' ); ?></option>
			</select>

			<button type="submit" class="button"><?php esc_html_e( 'Export', 'lantern-forms' ); ?></button>
		</form>
		<?php
	}
}
