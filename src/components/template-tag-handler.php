...
class Template_Tag_Handler {
	...
	private function validate_tag_data( $data ) {
		if ( ! is_array( $data ) ) {
			...
			throw new InvalidArgumentException( sprintf( __( 'Invalid template tag %1$s for handler %2$s.', 'lantern-forms' ), $slug, $this->slug ) );
		}
	...
	private function validate_tag_args( $tag_args, $tag_args_definition ) {
		if ( count( $tag_args ) !== count( $tag_args_definition ) ) {
			...
			throw new InvalidArgumentException( sprintf( __( 'Invalid template tag arguments passed to handler %s.', 'lantern-forms' ), $this->slug ) );
		}
			...
			switch ( $type ) {
				...
				default:
					if ( ! class_exists( $type ) && ! interface_exists( $type ) ) {
						/* translators: %s: template tag handler slug */
						throw new InvalidArgumentException( sprintf( __( 'Invalid template tag arguments definition for handler %s.', 'lantern-forms' ), $this->slug ) );
					}
			}
		}
