...
class Legacy_Upgrades extends Service {
	...
	public function maybe_upgrade_legacy_form_attachment_statuses() {
		...
		$taxonomy_slug = lantern()->taxonomies()->get_attachment_taxonomy_slug();
		...
		$form_attachment_ids = $wpdb->get_col( $wpdb->prepare( "SELECT ID FROM $wpdb->posts WHERE post_type = %s AND post_status = %s LIMIT 50", 'attachment', 'lantern-forms-upload' ) );
		...
		$form_attachment_ids = $wpdb->get_col( $wpdb->prepare( "SELECT ID FROM $wpdb->posts WHERE post_type = %s AND post_status = %s LIMIT 1", 'attachment', 'lantern-forms-upload' ) );
