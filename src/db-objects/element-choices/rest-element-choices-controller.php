...
namespace awsmug\Lantern_Forms\DB_Objects\Element_Choices;
...
class REST_Element_Choices_Controller extends REST_Models_Controller {
	...
	public function get_item_schema() {
		...
		$schema['properties']['element_id'] = array(
			'description' => __( 'ID of the element this element choice belongs to.', 'lantern-forms' ),
			'type'        => 'integer',
			'minimum'     => 1,
		...
		$schema['properties']['sort'] = array(
			'description' => __( 'Numeric value to determine the order within a list of multiple element choices.', 'lantern-forms' ),
			'type'        => 'integer',
			'context'     => array( 'view', 'edit', 'embed' ),
		);

		$schema['properties']['field'] = array(
			'description' => __( 'Element type field identifier the element choice is associated with, if not the main field.', 'lantern-forms' ),
			'type'        => 'string',
			'context'     => array( 'view', 'edit', 'embed' ),
		);
	...
	public function get_collection_params() {
		...
		$query_params['form_id'] = array(
			'description' => __( 'Limit result set to element choices associated with a specific form ID.', 'lantern-forms' ),
			'type'        => 'integer',
			'minimum'     => 1,
		);

		$query_params['container_id'] = array(
			'description' => __( 'Limit result set to element choices associated with a specific container ID.', 'lantern-forms' ),
			'type'        => 'integer',
			'minimum'     => 1,
		);

		$query_params['element_id'] = array(
			'description' => __( 'Limit result set to element choices associated with a specific element ID.', 'lantern-forms' ),
			'type'        => 'integer',
			'minimum'     => 1,
		);
