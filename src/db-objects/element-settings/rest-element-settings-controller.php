...
namespace awsmug\Lantern_Forms\DB_Objects\Element_Settings;
...
class REST_Element_Settings_Controller extends REST_Models_Controller {
	...
	public function get_item_schema() {
		...
		$schema['properties']['element_id'] = array(
			'description' => __( 'ID of the element this element setting belongs to.', 'lantern-forms' ),
			'type'        => 'integer',
			'minimum'     => 1,
		...
		$schema['properties']['name'] = array(
			'description' => __( 'Alphanumeric identifier for the element setting.', 'lantern-forms' ),
			'type'        => 'string',
			'context'     => array( 'view', 'edit', 'embed' ),
		);

		$schema['properties']['value'] = array(
			'description' => __( 'Value of the element setting.', 'lantern-forms' ),
			'type'        => 'string',
			'context'     => array( 'view', 'edit', 'embed' ),
		);
	...
	public function get_collection_params() {
		...
		$query_params['form_id'] = array(
			'description' => __( 'Limit result set to element settings associated with a specific form ID.', 'lantern-forms' ),
			'type'        => 'integer',
			'minimum'     => 1,
		);

		$query_params['container_id'] = array(
			'description' => __( 'Limit result set to element settings associated with a specific container ID.', 'lantern-forms' ),
			'type'        => 'integer',
			'minimum'     => 1,
		);

		$query_params['element_id'] = array(
			'description' => __( 'Limit result set to element settings associated with a specific element ID.', 'lantern-forms' ),
			'type'        => 'integer',
			'minimum'     => 1,
		);
