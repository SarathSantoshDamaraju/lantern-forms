...
class REST_Submissions_Controller extends REST_Models_Controller {
	...
	public function get_item_schema() {
		...
		$schema['properties']['form_id'] = array(
			'description' => __( 'ID of the form this submission belongs to.', 'lantern-forms' ),
			'type'        => 'integer',
			'minimum'     => 1,
		...
		$schema['properties']['user_id'] = array(
			'description' => __( 'ID of the user this submission belongs to, if any.', 'lantern-forms' ),
			'type'        => 'integer',
			'minimum'     => 0,
			'context'     => array( 'view', 'edit', 'embed' ),
			'arg_options' => array(
		...
		$schema['properties']['timestamp'] = array(
			'description' => __( 'UNIX timestamp for when the submission was created.', 'lantern-forms' ),
			'type'        => 'integer',
			'context'     => array( 'view', 'edit', 'embed' ),
		);

		$schema['properties']['remote_addr'] = array(
			'description' => __( 'IP address of who created the submission.', 'lantern-forms' ),
			'type'        => 'string',
			'context'     => array( 'view', 'edit', 'embed' ),
		);

		$schema['properties']['user_key'] = array(
			'description' => __( 'Key of who created the submission.', 'lantern-forms' ),
			'type'        => 'string',
			'context'     => array( 'view', 'edit', 'embed' ),
		);

		$schema['properties']['status'] = array(
			'description' => __( 'Status of the submission.', 'lantern-forms' ),
			'type'        => 'string',
	...
	public function get_collection_params() {
		...
		$query_params['form_id'] = array(
			'description' => __( 'Limit result set to submissions associated with a specific form ID.', 'lantern-forms' ),
			'type'        => 'integer',
			'minimum'     => 1,
		);

		$query_params['user_id'] = array(
			'description' => __( 'Limit result set to submissions associated with a specific user ID.', 'lantern-forms' ),
			'type'        => 'integer',
			'minimum'     => 1,
		);
