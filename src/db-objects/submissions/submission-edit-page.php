...
class Submission_Edit_Page extends Model_Edit_Page {
	...
	protected function render_header() {
		...
		if ( ! empty( $this->model->$primary_property ) ) {
			...
			$this->title = sprintf( __( 'Edit Submission %s', 'lantern-forms' ), '#' . $this->model->$primary_property );
		}
	...
	protected function get_tabs() {
		$tabs = array(
			'general' => array(
				'title' => _x( 'General', 'submission edit page tab', 'lantern-forms' ),
			),
		);
			...
			if ( $submission ) {
				...
				if ( $form ) {
					$tabs['form_input'] = array(
						'title'       => _x( 'Form Input', 'submission edit page tab', 'lantern-forms' ),
						'description' => __( 'Here you can edit the individual input values the user provided for the submission.', 'lantern-forms' ),
					);
				}
			}
		}
	...
	protected function get_sections() {
		$sections = array(
			'associated_data'     => array(
				'tab'   => 'general',
				'title' => _x( 'Associated Data', 'submission edit page section', 'lantern-forms' ),
			),
			'identification_data' => array(
				'tab'   => 'general',
				'title' => _x( 'Identification Data', 'submission edit page section', 'lantern-forms' ),
			),
		);
	...
	protected function get_fields() {
		$fields = array(
			'form_id'     => array(
				...
				'type'         => 'autocomplete',
				'label'        => __( 'Form', 'lantern-forms' ),
				'description'  => __( 'Specify the form this should be a submission for.', 'lantern-forms' ),
				'autocomplete' => array(
					'rest_placeholder_search_route' => 'lantern/v1/forms?search=%search%',
					'rest_placeholder_label_route'  => 'lantern/v1/forms/%value%',
					'value_generator'               => '%id%',
			...
			'user_id'     => array(
				'section'      => 'associated_data',
				'type'         => 'autocomplete',
				'label'        => __( 'User', 'lantern-forms' ),
				'description'  => __( 'Specify the user who should be associated with this submission.', 'lantern-forms' ),
				'autocomplete' => array(
			...
			'remote_addr' => array(
				...
				'type'          => 'text',
				'label'         => __( 'IP Address', 'lantern-forms' ),
				'description'   => __( 'Specify the IP address where this submission should be sent from.', 'lantern-forms' ),
			...
			'user_key'    => array(
				'section'       => 'identification_data',
				'type'          => 'text',
				'label'         => __( 'Key', 'lantern-forms' ),
				'description'   => __( 'Specify the key identifying the submission creator.', 'lantern-forms' ),
				'input_classes' => array( 'regular-text' ),
			),
		);
