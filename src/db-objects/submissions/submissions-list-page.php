...
class Submissions_List_Page extends Models_List_Page {
	...
	public function __construct( $slug, $manager, $model_manager ) {
		...
		if ( ! empty( $page ) && $slug === $page && ! empty( $form_id ) ) {
			...
			$this->title = sprintf( __( 'Submissions for form &#8220;%s&#8221;', 'lantern-forms' ), get_the_title( $form_id ) );
		}
	}
	...
	protected function render_header() {
		...
		<?php if ( ! empty( $form_id ) ) : ?>
			<a href="<?php echo esc_url( $this->url ); ?>" class="page-title-action"><?php echo esc_html_x( 'Back to Overview', 'submission admin list', 'lantern-forms' ); ?></a>
