...
class REST_Forms_Controller extends REST_Models_Controller {
	...
	public function get_item_schema() {
		...
		$schema['properties']['status'] = array(
			'description' => __( 'Status of the form.', 'lantern-forms' ),
			'type'        => 'string',
			'enum'        => array_keys(
				get_post_stati(
					array(
		...
		$schema['properties']['timestamp'] = array(
			'description' => __( 'UNIX timestamp for when the form was created.', 'lantern-forms' ),
			'type'        => 'integer',
			'context'     => array( 'view', 'edit', 'embed' ),
		);

		$schema['properties']['timestamp_modified'] = array(
			'description' => __( 'UNIX timestamp for when the form was last modified.', 'lantern-forms' ),
			'type'        => 'integer',
			'context'     => array( 'view', 'edit', 'embed' ),
		);
	...
	public function get_collection_params() {
		...
		$query_params['status'] = array(
			'description'       => __( 'Limit result set to forms with one or more specific statuses.', 'lantern-forms' ),
			'type'              => 'array',
			'items'             => array(
				'type' => 'string',
				'enum' => array_keys(
	...
	public function sanitize_status_param( $statuses, $request, $parameter ) {
		...
		foreach ( $statuses as $status ) {
			...
			if ( current_user_can( $post_type_obj->cap->edit_posts ) ) {
				...
				return new WP_Error( 'rest_forbidden_status', __( 'Status is forbidden.', 'lantern-forms' ), array( 'status' => rest_authorization_required_code() ) );
			}
		}
