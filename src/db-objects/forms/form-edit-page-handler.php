...
namespace awsmug\Lantern_Forms\DB_Objects\Forms;

use awsmug\Lantern_Forms\Assets;
...
class Form_Edit_Page_Handler {
	...
	public function add_meta_box( $id, $args ) {
		...
		$this->meta_boxes[ $id ]['field_manager'] = new Field_Manager(
			...
			array(
				...
				'field_required_markup'      => '<span class="screen-reader-text">' . _x( '(required)', 'field required indicator', 'lantern-forms' ) . '</span><span class="lantern-required-indicator" aria-hidden="true">*</span>',
			)
		);
	}
	...
	public function update_meta_values( $meta_box_id, $values ) {
		...
		if ( ! $this->form_manager->update_meta( $this->current_form->id, $meta_box_id, $values ) ) {
			...
			return new WP_Error( 'cannot_update_values', sprintf( __( 'An unknown error occurred while trying to save %s data.', 'lantern-forms' ), $meta_box_title ) );
		}
	...
	public function action_duplicate_form() {
		...
		if ( empty( $form_id ) ) {
			wp_die( esc_html__( 'Missing form ID.', 'lantern-forms' ), '', 400 );
		}

		if ( empty( $nonce ) ) {
			wp_die( esc_html__( 'Missing nonce.', 'lantern-forms' ), '', 400 );
		}

		if ( ! wp_verify_nonce( $nonce, $this->form_manager->get_prefix() . 'duplicate_form_' . $form_id ) ) {
			wp_die( esc_html__( 'Invalid nonce.', 'lantern-forms' ), '', 403 );
		}

		$form = $this->form_manager->get( $form_id );
		if ( ! $form ) {
			wp_die( esc_html__( 'Invalid form ID.', 'lantern-forms' ), '', 404 );
		}
		...
		if ( is_wp_error( $new_form ) ) {
			$feedback = array(
				...
				'message' => sprintf( __( 'The form &#8220;%1$s&#8221; could not be duplicated: %2$s', 'lantern-forms' ), $form->title, $new_form->get_error_message() ),
			);
		} else {
			$feedback = array(
				'type'    => 'success',
				/* translators: 1: form title, 2: new form edit URL */
				'message' => sprintf( __( 'The form &#8220;%1$s&#8221; was duplicated successfully. <a href="%2$s">View the duplicate</a>', 'lantern-forms' ), $form->title, get_edit_post_link( $new_form->id ) ),
			);
		}
	...
	public function maybe_show_form_save_feedback( $post ) {
		...
		?>
		<div class="lantern-notice notice notice-error">
			<p><?php esc_html_e( 'Some errors occurred while trying to save the form:', 'lantern-forms' ); ?></p>
			<ul>
				<?php
	...
	public function maybe_add_duplicate_button( $output, $post_id, $new_title, $new_slug, $post ) {
		...
		return $output . ' <a class="button button-small" href="' . esc_url( $url ) . '">' . esc_html( _x( 'Duplicate Form', 'action', 'lantern-forms' ) ) . '</a>';
	}
	...
	public function maybe_add_submissions_button( $output, $post_id, $new_title, $new_slug, $post ) {
		...
		$url = add_query_arg( 'form_id', $post_id, lantern()->admin_pages()->get( 'list_submissions' )->url );

		return $output . ' <a class="button button-small" href="' . esc_url( $url ) . '">' . esc_html( _x( 'View Form Submissions', 'action', 'lantern-forms' ) ) . '</a>';
	}
	...
	public function maybe_render_shortcode( $post ) {
		...
		<div class="misc-pub-section form-shortcode">
			<label for="<?php echo esc_attr( $id_attr ); ?>"><?php esc_html_e( 'Form Shortcode:', 'lantern-forms' ); ?></label>
			...
			<button type="button" class="clipboard-button button" data-clipboard-target="#<?php echo esc_attr( $id_attr ); ?>">
				<?php $this->form_manager->assets()->render_icon( 'lantern-icon-clippy', __( 'Copy to clipboard', 'lantern-forms' ) ); ?>
			</button>
		</div>
		<?php
	}
	...
	private function render_form_canvas( $form ) {
		?>
		<div id="lantern-form-canvas" class="lantern-form-canvas">
			<div class="lantern-form-canvas-header lantern-form-canvas-tabs" role="tablist">
				<button type="button" class="lantern-form-canvas-tab add-button is-active" disabled="disabled">
					<span aria-hidden="true">+</span><span class="screen-reader-text"><?php esc_html_e( 'Add New Container', 'lantern-forms' ); ?></span>
				</button>
			</div>
			<div class="lantern-form-canvas-content">
				<div class="drag-drop-area is-empty">
					<div class="content loader-content hide-if-no-js">
						<?php esc_html_e( 'Loading form builder...', 'lantern-forms' ); ?>
						<span class="spinner is-active"></span>
					</div>
					<div class="lantern-notice notice-warning hide-if-js">
						<p>
							<?php esc_html_e( 'It seems you have disabled JavaScript in your browser. Lantern Forms requires JavaScript in order to edit your forms.', 'lantern-forms' ); ?>
						</p>
					</div>
				</div>
			</div>
			<div class="lantern-form-canvas-footer"></div>
		</div>
		<?php
	}
	...
	private function add_meta_boxes( $form ) {
		...
		$meta_box_cb = function( $post, $box ) {
			...
			?>
			<h3 class="lantern-metabox-tab-wrapper" role="tablist">
				<?php foreach ( $tabs as $id => $args ) : ?>
					<a id="<?php echo esc_attr( $tab_id_prefix . $id ); ?>" class="lantern-metabox-tab" href="<?php echo esc_attr( '#' . $tabpanel_id_prefix . $id ); ?>" aria-controls="<?php echo esc_attr( $tabpanel_id_prefix . $id ); ?>" aria-selected="<?php echo $first ? 'true' : 'false'; ?>" role="tab">
			...
			<?php $first = true; ?>
			<?php foreach ( $tabs as $id => $args ) : ?>
				<div id="<?php echo esc_attr( $tabpanel_id_prefix . $id ); ?>" class="lantern-metabox-tab-panel" aria-labelledby="<?php echo esc_attr( $tab_id_prefix . $id ); ?>" aria-hidden="<?php echo $first ? 'false' : 'true'; ?>" role="tabpanel">
					<?php
	...
	private function print_templates() {
		?>
		<script type="text/html" id="tmpl-lantern-failure">
			<div class="lantern-notice notice-error">
				<p>
					<strong><?php esc_html_e( 'Error:', 'lantern-forms' ); ?></strong>
					{{ data.message }}
				</p>
			</div>
		</script>

		<script type="text/html" id="tmpl-lantern-form-canvas">
			<div class="lantern-form-canvas-header lantern-form-canvas-tabs">
				<button type="button" class="lantern-form-canvas-tab add-button">
					<span aria-hidden="true">+</span><span class="screen-reader-text"><?php esc_html_e( 'Add New Container', 'lantern-forms' ); ?></span>
				</button>
			</div>
			<div class="lantern-form-canvas-content">
				<div class="lantern-form-canvas-panel add-panel">
					<div class="drag-drop-area is-empty">
						<div class="content"><?php esc_html_e( 'Click the button above to add your first container', 'lantern-forms' ); ?></div>
					</div>
				</div>
			</div>
			<div class="lantern-form-canvas-footer"></div>
		...
		<script type="text/html" id="tmpl-lantern-container-tab">
			<span>{{ data.label }}</span>
		</script>

		<script type="text/html" id="tmpl-lantern-container-panel">
			...
			<div class="add-element-wrap">
				<div class="add-element-toggle-wrap">
					<button type="button" class="add-element-toggle button">
						<?php esc_html_e( 'Add element', 'lantern-forms' ); ?>
					</button>
				</div>
			</div>
		...
		<script type="text/html" id="tmpl-lantern-container-footer-panel">
			<button type="button" class="button-link button-link-delete delete-container-button">
				<?php esc_html_e( 'Delete Page', 'lantern-forms' ); ?>
			</button>
		</script>

		<script type="text/html" id="tmpl-lantern-element">
			<div class="lantern-element-header">
				<# if ( ! _.isEmpty( data.type.icon_css_class ) ) { #>
					<span class="lantern-element-header-icon {{ data.type.icon_css_class }}" aria-hidden="true"></span>
				<# } else if ( ! _.isEmpty( data.type.icon_svg_id ) ) { #>
					<svg class="lantern-icon lantern-element-header-icon" aria-hidden="true" role="img">
						<use href="#{{ data.type.icon_svg_id }}" xlink:href="#{{ data.type.icon_svg_id }}"></use>
					</svg>
				<# } else { #>
					<img class="lantern-element-header-icon" src="{{ data.type.icon_url }}" alt="">
				<# } #>
				<span class="lantern-element-header-title">
					{{ ! _.isEmpty( data.elementHeader ) ? data.elementHeader : data.type.title }}
				</span>
				<button type="button" class="lantern-element-expand-button" aria-controls="lantern-element-{{ data.id }}-content" aria-expanded="{{ data.active ? 'true' : 'false' }}">
					<span class="lantern-element-expand-button-icon" aria-hidden="true"></span><span class="screen-reader-text">{{ data.active ? '<?php esc_html_e( 'Hide Content', 'lantern-forms' ); ?>' : '<?php esc_html_e( 'Show Content', 'lantern-forms' ); ?>' }}</span>
				</button>
			</div>
			<div id="lantern-element-{{ data.id }}-content" class="{{ data.active ? 'lantern-element-content is-expanded' : 'lantern-element-content' }}" role="region">
				<div class="lantern-element-content-main">
					<div class="lantern-element-content-tabs"></div>
					<div class="lantern-element-content-panels"></div>
				</div>
				<div class="lantern-element-content-footer">
					<button type="button" class="button-link button-link-delete delete-element-button">
						<?php esc_html_e( 'Delete Element', 'lantern-forms' ); ?>
					</button>
				</div>
			</div>
		...
		<script type="text/html" id="tmpl-lantern-element-section-tab">
			<button type="button" id="element-tab-{{ data.elementId }}-{{ data.slug }}" class="lantern-element-content-tab lantern-element-content-tab-{{ data.slug }}" data-slug="{{ data.slug }}" aria-controls="element-panel-{{ data.elementId }}-{{ data.slug }}" aria-selected="{{ data.active ? 'true' : 'false' }}" role="tab">
				{{ data.title }}
			</button>
		</script>

		<script type="text/html" id="tmpl-lantern-element-section-panel">
			<div id="element-panel-{{ data.elementId }}-{{ data.slug }}" class="lantern-element-content-panel lantern-element-content-panel-{{ data.slug }}" aria-labelledby="element-tab-{{ data.elementId }}-{{ data.slug }}" aria-hidden="{{ data.active ? 'false' : 'true' }}" role="tabpanel">
				<table class="lantern-element-fields form-table"></table>
			</div>
		</script>

		<script type="text/html" id="tmpl-lantern-element-field">
		...
		<script type="text/html" id="tmpl-lantern-add-element-frame">
		...
		<script type="text/html" id="tmpl-lantern-element-types-browser">
			<div class="lantern-element-types">
				<# _.each( data.elementTypes, function( elementType ) { #>
					<div class="lantern-element-type lantern-element-type-{{ elementType.slug }}{{ elementType.slug === data.selectedElementType ? ' is-selected' : '' }}" data-slug="{{ elementType.slug }}" tabindex="0">
						<div class="lantern-element-type-header">
							<# if ( ! _.isEmpty( elementType.icon_css_class ) ) { #>
								<span class="lantern-element-type-header-icon {{ elementType.icon_css_class }}" aria-hidden="true"></span>
							<# } else if ( ! _.isEmpty( elementType.icon_svg_id ) ) { #>
								<svg class="lantern-icon lantern-element-type-header-icon" aria-hidden="true" role="img">
									<use href="#{{ elementType.icon_svg_id }}" xlink:href="#{{ elementType.icon_svg_id }}"></use>
								</svg>
							<# } else { #>
								<img class="lantern-element-type-header-icon" src="{{ elementType.icon_url }}" alt="">
							<# } #>
							<span class="lantern-element-type-header-title">
								{{ elementType.title }}
							</span>
						</div>
						<div class="lantern-element-type-content">
