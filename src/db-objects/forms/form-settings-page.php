...
class Form_Settings_Page extends Tabbed_Settings_Page {
	...
	public function __construct( $slug, $manager, $form_manager ) {
		...
		$this->title      = __( 'Settings', 'lantern-forms' );
	...
	protected function render_form_content( $current_tab_id, $current_subtab_id ) {
		if ( $this->manager->get_prefix() . 'general_settings' === $current_tab_id ) {
			?>
			<div class="welcome-to-lantern">
				<h3><?php esc_html_e( 'Welcome to Lantern Forms!', 'lantern-forms' ); ?></h3>
				<p><?php esc_html_e( 'You want to build forms in an easy way? Lantern Forms will help you do it quickly, yet with tons of options.', 'lantern-forms' ); ?></p>
			</div>
			<?php
		}
		...
		?>
		<div class="lantern-form-content <?php echo $use_subtabs ? 'tabbed' : 'no-tabs'; ?>">

			<?php if ( $use_subtabs ) : ?>
				<div class="lantern-subtab-wrapper" role="tablist">
					<?php foreach ( $subtabs as $subtab_id => $subtab_args ) : ?>
						...
						?>
						<a id="<?php echo esc_attr( 'lantern-subtab-label-' . $subtab_id ); ?>" class="lantern-subtab" href="<?php echo esc_url( $url ); ?>" aria-controls="<?php echo esc_attr( 'lantern-subtab-' . $subtab_id ); ?>" aria-selected="<?php echo $subtab_id === $current_subtab_id ? 'true' : 'false'; ?>" role="tab">
			...
			<?php foreach ( $subtabs as $subtab_id => $subtab_args ) : ?>
				<?php
				$atts = $use_subtabs ? ' aria-labelledby="' . esc_attr( 'lantern-subtab-label-' . $subtab_id ) . '" aria-hidden="' . ( $subtab_id === $current_subtab_id ? 'false' : 'true' ) . '" role="tabpanel"' : '';
				?>
				<div id="<?php echo esc_attr( 'lantern-subtab-' . $subtab_id ); ?>" class="lantern-subtab-panel"<?php echo $atts; /* phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped */ ?>>
	...
	protected function add_page_content() {
		 ...
		 * @since 1.0.0
		 *
		 * @param awsmug\Lantern_Forms\DB_Objects\Forms\Form_Settings_Page $settings_page The settings page instance.
		 */
	...
	protected function get_tabs() {
		$tabs = array(
			'general_settings'   => array(
				'title'            => _x( 'General', 'form settings', 'lantern-forms' ),
				'rest_description' => _x( 'Lantern Forms general settings.', 'REST API description', 'lantern-forms' ),
			),
			'extension_settings' => array(
				'title'            => _x( 'Extensions', 'form settings', 'lantern-forms' ),
				'rest_description' => _x( 'Lantern Forms extension settings.', 'REST API description', 'lantern-forms' ),
			),
		);
	...
	protected function get_sections() {
		$sections = array(
			'modules'       => array(
				'subtab' => 'general',
				'title'  => _x( 'Modules', 'form settings', 'lantern-forms' ),
			),
			'form_behavior' => array(
				'subtab' => 'general',
				'title'  => __( 'Form Behavior', 'lantern-forms' ),
			),
			'advanced'      => array(
				'subtab' => 'general',
				'title'  => _x( 'Advanced', 'form settings', 'lantern-forms' ),
			),
		);
	...
	protected function get_fields() {
		...
		$modules = array();
		foreach ( lantern()->modules()->get_all() as $slug => $module ) {
			$modules[ $slug ] = $module->get_title();
		}
		$default_modules = array_keys( $modules );

		$default_slug = _x( 'forms', 'default form rewrite slug', 'lantern-forms' );

		$fields = array(
			'modules' => array(
				'section'     => 'modules',
				'type'        => 'multibox',
				'label'       => __( 'Active Modules', 'lantern-forms' ),
				'description' => __( 'If you do not need all of these modules, you can disable them here.', 'lantern-forms' ),
				'choices'     => $modules,
			...
			'slug'    => array(
				...
				'type'        => 'text',
				'label'       => __( 'Slug', 'lantern-forms' ),
				/* translators: %s: sample form permalink URL */
				'description' => sprintf( __( 'The slug for permalinks (e.g. for a URL like %s).', 'lantern-forms' ), home_url( '/' ) . '<strong id="lantern-rewrite-slug-preview">' . ( ! empty( $options['slug'] ) ? $options['slug'] : $default_slug ) . '</strong>/my-contact-form/' ),
				'default'     => $default_slug,
				'required'    => true,
			),
		);

		$attachment_taxonomy_slug = lantern()->taxonomies()->get_attachment_taxonomy_slug();
		if ( ! empty( $attachment_taxonomy_slug ) ) {
			$attachment_taxonomy = get_taxonomy( $attachment_taxonomy_slug );
			if ( $attachment_taxonomy ) {
				$term_choices = array( '0' => _x( 'None', 'term choices', 'lantern-forms' ) );
				$terms        = get_terms(
					array(
				...
				$fields['attachment_taxonomy_term_id'] = array(
					...
					'label'       => sprintf( __( '%s Term', 'lantern-forms' ), $attachment_taxonomy->labels->singular_name ),
					'description' => __( 'The default term to use for form uploads.', 'lantern-forms' ),
					'default'     => '0',
		...
		$fields = array_merge(
			...
			array(
				'frontend_css'            => array(
					'section' => 'advanced',
					'type'    => 'checkbox',
					'label'   => __( 'Include Lantern Forms CSS on frontend?', 'lantern-forms' ),
					'default' => true,
				),
				'delete_submissions'      => array(
					'section'     => 'advanced',
					'type'        => 'checkbox',
					'label'       => __( 'Delete submission that have not completed after a certain amount of time?', 'lantern-forms' ),
					'description' => __( 'Enabling this setting can help keep your database cleaner by deleting submissions that have been started, but never completed.', 'lantern-forms' ),
					'default'     => false,
				),
				'delete_submissions_days' => array(
					'section'       => 'advanced',
					'type'          => 'number',
					'label'         => __( 'Submission Deletion', 'lantern-forms' ),
					'description'   => __( 'Specify the number of days after which incomplete submissions should be deleted.', 'lantern-forms' ),
					'min'           => 1,
					'step'          => 1,
					'default'       => 1,
					'unit'          => _x( 'day/s', 'field unit', 'lantern-forms' ),
				...
				'hard_uninstall'          => array(
					'section'     => 'advanced',
					'type'        => 'checkbox',
					'label'       => __( 'Perform a hard uninstall when the plugin is removed?', 'lantern-forms' ),
					'description' => __( '<strong>Use this setting with extreme caution</strong> as, when it is enabled, removing the plugin will remove all form content from your site forever.', 'lantern-forms' ),
					'default'     => false,
				),
			)
		);
