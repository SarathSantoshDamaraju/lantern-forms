...
class Form_List_Page_Handler {
	...
	public function maybe_adjust_table_columns( $columns ) {
		$new_columns = array(
			'form_shortcode'   => __( 'Shortcode', 'lantern-forms' ),
			'submission_count' => __( 'Submission Count', 'lantern-forms' ),
		);
	...
	public function insert_custom_row_actions( $actions, $form ) {
		...
		$actions[ $prefix . 'view_submissions' ] = sprintf(
			...
			add_query_arg( 'form_id', $form->id, lantern()->admin_pages()->get( 'list_submissions' )->url ),
			/* translators: %s: form title */
			esc_attr( sprintf( __( 'View submissions for &#8220;%s&#8221;', 'lantern-forms' ), get_the_title( $form->id ) ) ),
			_x( 'View Submissions', 'action', 'lantern-forms' )
		);
		...
		$actions[ $prefix . 'duplicate' ] = sprintf(
			...
			/* translators: %s: form title */
			esc_attr( sprintf( __( 'Duplicate &#8220;%s&#8221;', 'lantern-forms' ), get_the_title( $form->id ) ) ),
			_x( 'Duplicate', 'action', 'lantern-forms' )
		);
	...
	protected function render_custom_list_table_column( $column_name, $form ) {
		switch ( $column_name ) {
			...
			case 'submission_count':
				...
				if ( $count > 0 ) {
					$output = '<a href="' . esc_url( add_query_arg( 'form_id', $form->id, lantern()->admin_pages()->get( 'list_submissions' )->url ) ) . '">' . $output . '</a>';
				}
