...
use awsmug\Lantern_Forms\DB_Objects\Submission_Values\Submission_Value;
...
class Form_Frontend_Submission_Handler {
	...
	public function maybe_handle_form_submission() {
		if ( ! isset( $_POST['lantern_submission'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			return;
		}

		$data = wp_unslash( $_POST['lantern_submission'] ); // phpcs:ignore WordPress.Security

		$context = $this->detect_request_form_and_submission( $data );
		if ( is_wp_error( $context ) ) {
			// Always die when one of these strange errors happens.
			wp_die( wp_kses_data( $context->get_error_message() ), esc_html__( 'Form Submission Error', 'lantern-forms' ), 400 );
		}
		...
		if ( ! $verified || is_wp_error( $verified ) ) {
			if ( ! $verified ) {
				$verified = new WP_Error( 'cannot_verify_request', __( 'The request could not be verified.', 'lantern-forms' ) );
			}

			// Die only if the form error could not be set.
			if ( ! $this->set_form_error( $form, $verified ) ) {
				wp_die( wp_kses_data( $verified->get_error_message() ), esc_html__( 'Form Submission Error', 'lantern-forms' ), 403 );
			}
		} else {
			if ( ! $submission ) {
		...
		if ( $submission && ! empty( $submission->id ) ) {
			$redirect_url = add_query_arg( 'lantern_submission_id', $submission->id, $redirect_url );
		}
	...
	protected function handle_form_submission( $form, $submission, $data = array() ) {
		...
		if ( ! $container ) {
			$submission->add_error( 0, 'internal_error_container', __( 'Internal error: No current container is available for this form.', 'lantern-forms' ) );
		...
		if ( $going_back ) {
			...
			if ( $previous_container ) {
				...
				$submission->add_error( 0, 'internal_error_previous_container', __( 'Internal error: There is no previous container available.', 'lantern-forms' ) );
			}
		} else {
	...
	protected function detect_request_form_and_submission( $data ) {
		...
		if ( ! empty( $data['id'] ) ) {
			...
			if ( $submission ) {
				if ( 'completed' === $submission->status ) {
					return new WP_Error( 'submission_already_completed', __( 'Submission already completed.', 'lantern-forms' ) );
				}
		...
		if ( ! $form ) {
			if ( empty( $data['form_id'] ) ) {
				return new WP_Error( 'cannot_detect_form', __( 'Could not detect form.', 'lantern-forms' ) );
			}
	...
	protected function verify_request( $data, $form, $submission = null ) {
		if ( ! isset( $data['nonce'] ) ) {
			return new WP_Error( 'missing_nonce', __( 'Missing security nonce.', 'lantern-forms' ) );
		}

		if ( ! wp_verify_nonce( $data['nonce'], $this->get_nonce_action( $form, $submission ) ) ) {
			return new WP_Error( 'invalid_nonce', __( 'Invalid security nonce.', 'lantern-forms' ) );
		}
