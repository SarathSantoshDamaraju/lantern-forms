...
use awsmug\Lantern_Forms\DB_Objects\Containers\Container_Collection;
use awsmug\Lantern_Forms\DB_Objects\Containers\Element_Collection;
use awsmug\Lantern_Forms\DB_Objects\Submissions\Submission_Collection;
use awsmug\Lantern_Forms\DB_Objects\Participants\Participant_Collection;
use WP_Post;
use WP_Error;
use stdClass;
...
class Form extends Core_Model {
	...
	public function to_json( $include_meta = true ) {
		 ...
		 */
		$form_classes = apply_filters( "{$this->manager->get_prefix()}form_classes", array( 'lantern-form' ), $this );
		 ...
		 */
		$form_action_url = apply_filters( 'lantern_form_action_url', home_url( $wp->request ), (int) $this->original->ID );

		$data['form_attrs'] = array(
			'id'         => 'lantern-form-' . $this->original->ID,
	...
	public function duplicate( $as_draft = true, $new_title = '' ) {
		if ( empty( $this->original->ID ) ) {
			return new WP_Error( 'form_post_not_exist', __( 'The form post does not exist in the database.', 'lantern-forms' ) );
		}
		...
		if ( ! empty( $new_title ) ) {
			...
			$post_data['post_title'] = sprintf( _x( 'Copy of %s', 'duplicated form title', 'lantern-forms' ), $post_data['post_title'] );
		}
