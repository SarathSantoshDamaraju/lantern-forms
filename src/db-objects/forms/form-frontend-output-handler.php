...
use awsmug\Lantern_Forms\DB_Objects\Containers\Container;
use awsmug\Lantern_Forms\Error;
...
class Form_Frontend_Output_Handler {
	...
	public function maybe_get_form_content( $content ) {
		...
		$submission = null;
		if ( isset( $_GET['lantern_submission_id'] ) ) { // WPCS: CSRF OK.
			$submission = $this->form_manager->get_child_manager( 'submissions' )->get( absint( $_GET['lantern_submission_id'] ) ); // WPCS: CSRF OK.
	...
	public function get_shortcode_content( $atts ) {
		...
		if ( 'iframe' === $atts['show'] ) {
			...
			if ( isset( $_GET['lantern_submission_id'] ) ) { // WPCS: CSRF OK.
				$url = add_query_arg( 'lantern_submission_id', absint( $_GET['lantern_submission_id'] ), $url ); // WPCS: CSRF OK.
			}
	...
	protected function render_form_content( $form, $submission = null ) {
		...
		if ( ! $can_access_form ) {
			$message = $submission ? __( 'You are not allowed to access this form submission.', 'lantern-forms' ) : __( 'You are not allowed to access this form.', 'lantern-forms' );
		...
		if ( $submission && 'completed' === $submission->status ) {
			 ...
			 */
			$success_message = apply_filters( "{$prefix}form_submission_success_message", __( 'Thank you for submitting!', 'lantern-forms' ), $form->id );
		...
		if ( ! $container ) {
			$this->print_notice( __( 'No container exists for this form.', 'lantern-forms' ), 'error' );
			return;
		}
		...
		$template_data['hidden_fields']  = '<input type="hidden" name="lantern_submission[nonce]" value="' . wp_create_nonce( $this->get_nonce_action( $form, $submission ) ) . '">';
		$template_data['hidden_fields'] .= '<input type="hidden" name="lantern_submission[form_id]" value="' . esc_attr( $form->id ) . '">';
		if ( $submission ) {
			$template_data['hidden_fields'] .= '<input type="hidden" name="lantern_submission[id]" value="' . esc_attr( $submission->id ) . '">';
		}
		if ( ! is_archive() && in_the_loop() && (int) get_the_ID() !== $form->id ) {
			$template_data['hidden_fields'] .= '<input type="hidden" name="lantern_submission[original_form_id]" value="' . esc_attr( get_the_ID() ) . '">';
		}
		 ...
		 * @since 1.0.0
		 *
		 * @param string $button_class Button CSS class. Default 'lantern-button'.
		 */
		$button_class = apply_filters( "{$prefix}form_button_class", 'lantern-button' );
		...
		if ( $this->has_next_container( $form, $submission ) ) {
			 ...
			 * @param int    $form_id           Form ID.
			 */
			$next_button_label = apply_filters( "{$prefix}form_button_next_step_label", _x( 'Next Step', 'button label', 'lantern-forms' ), $form->id );

			$template_data['navigation']['next_button'] = array(
				'label' => $next_button_label,
				'attrs' => array(
					'type'  => 'submit',
					'name'  => 'lantern_submission[action]',
					'value' => 'next',
					'class' => $button_class,
				),
			);
			 ...
			 * @param int    $form_id             Form ID.
			 */
			$submit_button_label = apply_filters( "{$prefix}form_button_submit_label", _x( 'Submit', 'button label', 'lantern-forms' ), $form->id );
			 ...
			 * @since 1.0.0
			 *
			 * @param string $button_primary_class Primary button CSS class. Default 'lantern-button-primary'.
			 */
			$button_primary_class = apply_filters( "{$prefix}form_button_primary_class", 'lantern-button-primary' );
		...
		if ( $this->has_previous_container( $form, $submission ) ) {
			 ...
			 * @param int    $form_id           Form ID.
			 */
			$prev_button_label = apply_filters( "{$prefix}form_button_prev_step_label", _x( 'Previous Step', 'button label', 'lantern-forms' ), $form->id );
		...
		if ( $submission->has_errors( 0 ) ) {
			...
			if ( 1 === count( $global_errors ) ) {
				...
				<div class="<?php echo esc_attr( $this->get_notice_class( 'error' ) ); ?>">
					<p><?php esc_html_e( 'Some errors occurred while trying to submit the form:', 'lantern-forms' ); ?></p>
					<ul>
			...
			$this->print_notice( __( 'Some errors occurred while trying to submit the form.', 'lantern-forms' ), 'error' );
		}
	}
	...
	protected function get_notice_class( $type = 'warning' ) {
		 ...
		 * @since 1.0.0
		 *
		 * @param string $notice_class Notice CSS class. Default 'lantern-notice lantern-{$type}-notice'.
		 * @param string $type         Notice type.
		 */
		return apply_filters( "{$this->form_manager->get_prefix()}form_notice_class", "lantern-notice lantern-{$type}-notice", $type );
	}
