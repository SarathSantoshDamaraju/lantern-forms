...
namespace awsmug\Lantern_Forms\DB_Objects\Submission_Values;
...
class REST_Submission_Values_Controller extends REST_Models_Controller {
	...
	public function get_item_schema() {
		...
		$schema['properties']['submission_id'] = array(
			'description' => __( 'ID of the submission this submission value belongs to.', 'lantern-forms' ),
			'type'        => 'integer',
			'minimum'     => 1,
		...
		$schema['properties']['element_id'] = array(
			'description' => __( 'ID of the element this submission value refers to.', 'lantern-forms' ),
			'type'        => 'integer',
			'minimum'     => 1,
			'context'     => array( 'view', 'edit', 'embed' ),
			'arg_options' => array(
		...
		$schema['properties']['field'] = array(
			'description' => __( 'Element type field identifier the submission value is associated with, if not the main element field.', 'lantern-forms' ),
			'type'        => 'string',
			'context'     => array( 'view', 'edit', 'embed' ),
		);

		$schema['properties']['value'] = array(
			'description' => __( 'The submission value.', 'lantern-forms' ),
			'type'        => 'string',
			'context'     => array( 'view', 'edit', 'embed' ),
		);
	...
	public function get_collection_params() {
		...
		$query_params['form_id'] = array(
			'description' => __( 'Limit result set to submission values associated with a specific form ID.', 'lantern-forms' ),
			'type'        => 'integer',
			'minimum'     => 1,
		);

		$query_params['submission_id'] = array(
			'description' => __( 'Limit result set to submission values associated with a specific submission ID.', 'lantern-forms' ),
			'type'        => 'integer',
			'minimum'     => 1,
		);

		$query_params['element_id'] = array(
			'description' => __( 'Limit result set to submission values referring to a specific element ID.', 'lantern-forms' ),
			'type'        => 'integer',
			'minimum'     => 1,
		);
