...
namespace awsmug\Lantern_Forms\DB_Objects\Containers;
...
class REST_Containers_Controller extends REST_Models_Controller {
	...
	public function get_item_schema() {
		...
		$schema['properties']['form_id'] = array(
			'description' => __( 'ID of the form this container belongs to.', 'lantern-forms' ),
			'type'        => 'integer',
			'minimum'     => 1,
		...
		$schema['properties']['sort'] = array(
			'description' => __( 'Numeric value to determine the order within a list of multiple containers.', 'lantern-forms' ),
			'type'        => 'integer',
			'context'     => array( 'view', 'edit', 'embed' ),
		);
	...
	public function get_collection_params() {
		...
		$query_params['form_id'] = array(
			'description' => __( 'Limit result set to containers associated with a specific form ID.', 'lantern-forms' ),
			'type'        => 'integer',
			'minimum'     => 1,
		);
