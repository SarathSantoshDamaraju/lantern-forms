...
use awsmug\Lantern_Forms\DB_Objects\REST_Embed_Limits_Trait;
use awsmug\Lantern_Forms\DB_Objects\Elements\Element_Types\REST_Element_Types_Controller;
use WP_REST_Request;
use WP_Error;
...
class REST_Elements_Controller extends REST_Models_Controller {
	...
	public function get_item_schema() {
		...
		$schema['properties']['container_id'] = array(
			'description' => __( 'ID of the container this element belongs to.', 'lantern-forms' ),
			'type'        => 'integer',
			'minimum'     => 1,
		...
		$schema['properties']['sort'] = array(
			'description' => __( 'Numeric value to determine the order within a list of multiple elements.', 'lantern-forms' ),
			'type'        => 'integer',
			'context'     => array( 'view', 'edit', 'embed' ),
		);

		$schema['properties']['type'] = array(
			'description' => __( 'Type of the element.', 'lantern-forms' ),
			'type'        => 'string',
	...
	public function get_collection_params() {
		...
		$query_params['form_id'] = array(
			'description' => __( 'Limit result set to elements associated with a specific form ID.', 'lantern-forms' ),
			'type'        => 'integer',
			'minimum'     => 1,
		);

		$query_params['container_id'] = array(
			'description' => __( 'Limit result set to elements associated with a specific container ID.', 'lantern-forms' ),
			'type'        => 'integer',
			'minimum'     => 1,
		);

		$query_params['type'] = array(
			'description'       => __( 'Limit result set to elements with one or more specific statuses.', 'lantern-forms' ),
			'type'              => 'array',
			'items'             => array(
				'type' => 'string',
				'enum' => array_keys( $this->manager->types()->get_all() ),
