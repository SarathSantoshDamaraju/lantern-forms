...
abstract class Element_Type {
	...
	public function __construct( $manager ) {
		...
		$this->settings_sections = array(
			'content'  => array(
				'title' => _x( 'Content', 'element type section', 'lantern-forms' ),
			),
			'settings' => array(
				'title' => _x( 'Settings', 'element type section', 'lantern-forms' ),
			),
		);
		...
		$this->settings_fields = array(
			'label' => array(
				'section'       => 'content',
				'type'          => 'text',
				'label'         => __( 'Label', 'lantern-forms' ),
				'description'   => __( 'Enter the form field label.', 'lantern-forms' ),
	...
	public function get_values( $element, $submission = null ) {
		...
		if ( has_filter( "{$this->manager->get_prefix()}allow_get_params" ) && isset( $_GET[ 'lantern_input_value_' . $element->id ] ) && ( is_array( $_GET[ 'lantern_input_value_' . $element->id ] ) || empty( $values['_main'] ) ) ) { // WPCS: CSRF OK.
			...
			if ( $container ) {
				...
				if ( $form ) {
					...
					if ( $allow_get_params ) {
						...
						$get_params = wp_unslash( $_GET[ 'lantern_input_value_' . $element->id ] ); // phpcs:ignore WordPress.Security
	...
	public function filter_json( $data, $element, $submission = null ) {
		...
		if ( ! empty( $settings['required'] ) && 'no' !== $settings['required'] ) {
			$required_indicator = '<span class="screen-reader-text">' . _x( '(required)', 'field required indicator', 'lantern-forms' ) . '</span><span class="lantern-required-indicator" aria-hidden="true">*</span>';
	...
	protected function add_placeholder_settings_field( $section = 'settings' ) {
		$this->settings_fields['placeholder'] = array(
			'section'       => $section,
			'type'          => 'text',
			'label'         => __( 'Placeholder', 'lantern-forms' ),
			'description'   => __( 'Placeholder text will be shown until data is being entered.', 'lantern-forms' ),
			'input_classes' => array( 'regular-text' ),
		);
	}
	...
	protected function add_description_settings_field( $section = 'settings' ) {
		$this->settings_fields['description'] = array(
			'section'       => $section,
			'type'          => 'textarea',
			'label'         => __( 'Description', 'lantern-forms' ),
			'description'   => __( 'The description will be shown below the element.', 'lantern-forms' ),
	...
	protected function add_required_settings_field( $section = 'settings' ) {
		$this->settings_fields['required'] = array(
			'section'     => $section,
			'type'        => 'radio',
			'label'       => __( 'Required?', 'lantern-forms' ),
			'choices'     => array(
				'yes' => __( 'Yes', 'lantern-forms' ),
				'no'  => __( 'No', 'lantern-forms' ),
			),
			'description' => __( 'Whether the user must input something.', 'lantern-forms' ),
			'default'     => 'no',
		);
	}
	...
	protected function add_css_classes_settings_field( $section = 'settings' ) {
		$this->settings_fields['css_classes'] = array(
			'section'       => $section,
			'type'          => 'text',
			'label'         => __( 'CSS Classes', 'lantern-forms' ),
			'description'   => __( 'Additional CSS Classes separated by whitespaces.', 'lantern-forms' ),
			'input_classes' => array( 'regular-text' ),
		);
	}
	...
	final protected function sanitize_settings_fields() {
		...
		foreach ( $this->settings_fields as $slug => $field ) {
			if ( empty( $field['section'] ) || ! isset( $this->settings_sections[ $field['section'] ] ) ) {
				...
				$this->manager->error_handler()->doing_it_wrong( get_class( $this ) . '::bootstrap()', sprintf( __( 'Invalid element type field section %s.', 'lantern-forms' ), esc_html( $field['section'] ) ), '1.0.0' );
			...
			if ( empty( $field['type'] ) || ! Field_Manager::is_field_type_registered( $field['type'] ) ) {
				...
				$this->manager->error_handler()->doing_it_wrong( get_class( $this ) . '::bootstrap()', sprintf( __( 'Invalid element type field type %s.', 'lantern-forms' ), esc_html( $field['type'] ) ), '1.0.0' );
				$invalid_fields[ $slug ] = true;
				continue;
			}

			if ( in_array( $field['type'], array( 'multiselect', 'multibox', 'group' ), true ) || empty( $field['is_choices'] ) && 'lanternchoices' === $field['type'] ) {
				/* translators: %s: field type slug */
				$this->manager->error_handler()->doing_it_wrong( get_class( $this ) . '::bootstrap()', sprintf( __( 'Disallowed element type field type %s.', 'lantern-forms' ), esc_html( $field['type'] ) ), '1.0.0' );
				$invalid_fields[ $slug ] = true;
				continue;
			}
			...
			if ( ! empty( $field['repeatable'] ) && empty( $field['is_choices'] ) ) {
				/* translators: %s: field type slug */
				$this->manager->error_handler()->doing_it_wrong( get_class( $this ) . '::bootstrap()', __( 'Disallowed repeatable element type field.', 'lantern-forms' ), '1.0.0' );
				$invalid_fields[ $slug ] = true;
				continue;
			}
