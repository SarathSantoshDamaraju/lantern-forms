...
class Element_Type_Manager extends Service {
	...
	public function __construct( $prefix, $services ) {
		...
		Field_Manager::register_field_type( 'lanternchoices', Choices_Field::class );
	...
	public function get( $slug ) {
		if ( ! $this->has( $slug ) ) {
			...
			return new Error( $this->get_prefix() . 'element_type_not_exist', sprintf( __( 'An element type with the slug %s does not exist.', 'lantern-forms' ), $slug ), __METHOD__, '1.0.0' );
		}
	...
	public function register( $slug, $element_type_class_name ) {
		if ( ! did_action( 'init' ) ) {
			...
			return new Error( $this->get_prefix() . 'element_type_too_early', sprintf( __( 'The element type %1$s cannot be registered before the %2$s hook.', 'lantern-forms' ), $slug, '<code>init</code>' ), __METHOD__, '1.0.0' );
		}

		if ( $this->has( $slug ) ) {
			/* translators: %s: element type slug */
			return new Error( $this->get_prefix() . 'element_type_already_exist', sprintf( __( 'An element type with the slug %s already exists.', 'lantern-forms' ), $slug ), __METHOD__, '1.0.0' );
		}

		if ( ! class_exists( $element_type_class_name ) ) {
			/* translators: %s: element type class name */
			return new Error( $this->get_prefix() . 'element_type_class_not_exist', sprintf( __( 'The class %s does not exist.', 'lantern-forms' ), $element_type_class_name ), __METHOD__, '1.0.0' );
		}

		if ( ! is_subclass_of( $element_type_class_name, Element_Type::class ) ) {
			/* translators: %s: element type class name */
			return new Error( $this->get_prefix() . 'element_type_class_not_allowed', sprintf( __( 'The class %s is not allowed for a element type.', 'lantern-forms' ), $element_type_class_name ), __METHOD__, '1.0.0' );
		}
	...
	public function unregister( $slug ) {
		...
		if ( isset( $this->default_element_types[ $slug ] ) ) {
			/* translators: %s: element type slug */
			return new Error( $this->get_prefix() . 'element_type_is_default', sprintf( __( 'The default element type %s cannot be unregistered.', 'lantern-forms' ), $slug ), __METHOD__, '1.0.0' );
		}
