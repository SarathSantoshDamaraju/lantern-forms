...
use awsmug\Lantern_Forms\DB_Objects\Elements\Element_Types\Non_Input_Element_Type_Trait;
...
class Content extends Element_Type implements Non_Input_Element_Type_Interface {
	...
	protected function bootstrap() {
		$this->slug        = 'content';
		$this->title       = __( 'Content', 'lantern-forms' );
		$this->description = __( 'A non-input element to display custom content.', 'lantern-forms' );
		$this->icon_svg_id = 'lantern-icon-content';

		$this->settings_fields['label']['type']          = 'wysiwyg';
		$this->settings_fields['label']['label']         = __( 'Content', 'lantern-forms' );
		$this->settings_fields['label']['description']   = __( 'Enter the content to display.', 'lantern-forms' );
