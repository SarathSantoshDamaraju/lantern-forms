...
class Media extends Element_Type {
	...
	protected function bootstrap() {
		$this->slug        = 'media';
		$this->title       = __( 'Media', 'lantern-forms' );
		$this->description = __( 'An element to upload a file to the media library.', 'lantern-forms' );
		$this->icon_svg_id = 'lantern-icon-media';
		...
		$this->settings_fields['file_type'] = array(
			'section'     => 'settings',
			'type'        => 'select',
			'label'       => __( 'Valid file types', 'lantern-forms' ),
			'description' => __( 'The file type the user is allowed to upload.', 'lantern-forms' ),
	...
	public function format_values_for_export( $values, $element, $export_format ) {
		...
		if ( ! empty( $value ) ) {
			...
			if ( $attachment ) {
				...
				$output = __( 'File deleted.', 'lantern-forms' );
			}
		} else {
			$output = __( 'No file uploaded.', 'lantern-forms' );
		}
	...
	public function validate_field( $value, $element, $submission ) {
		...
		if ( ! empty( $settings['required'] ) && 'no' !== $settings['required'] && empty( $value ) && ! $has_file ) {
			return $this->create_error( Element_Type::ERROR_CODE_REQUIRED, __( 'You must upload a file.', 'lantern-forms' ), $value );
		}
		...
		if ( $has_file ) {
			...
			$attachment_id = lantern()->form_uploads()->upload_file( $file_id, $submission, $form, $element->id, '_main', $mimes );
	...
	protected function get_type_dropdown_options() {
		$media_types = array(
			'type_image'       => __( 'Images (.jpg, .png, ...)', 'lantern-forms' ),
			'type_audio'       => __( 'Audio files (.wav, .mp3, ...)', 'lantern-forms' ),
			'type_video'       => __( 'Video files (.avi, .mpeg, ...)', 'lantern-forms' ),
			'type_document'    => __( 'Documents (.doc, .pdf, ...)', 'lantern-forms' ),
			'type_spreadsheet' => __( 'Spreadsheets (.xls, .numbers, ...)', 'lantern-forms' ),
			'type_interactive' => __( 'Interactive (.ppt, .swf, ...)', 'lantern-forms' ),
			'type_text'        => __( 'Text files (.txt, .csv, ...)', 'lantern-forms' ),
			'type_archive'     => __( 'File archives (.zip, .rar, ...)', 'lantern-forms' ),
			'type_code'        => __( 'Source code (.html, .js, ...)', 'lantern-forms' ),
		);
		...
		return array_merge( array( 'any' => __( 'Any', 'lantern-forms' ) ), $media_types, $media_sub_types );
	}
