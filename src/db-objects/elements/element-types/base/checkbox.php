...
class Checkbox extends Element_Type {
	...
	protected function bootstrap() {
		$this->slug        = 'checkbox';
		$this->title       = __( 'Checkbox', 'lantern-forms' );
		$this->description = __( 'A single checkbox element to toggle a value.', 'lantern-forms' );
		$this->icon_svg_id = 'lantern-icon-checkbox';
	...
	public function validate_field( $value, $element, $submission ) {
		...
		if ( ! empty( $settings['required'] ) && 'no' !== $settings['required'] && ! $value ) {
			return $this->create_error( Element_Type::ERROR_CODE_REQUIRED, __( 'You must check this box.', 'lantern-forms' ), $value );
		}
