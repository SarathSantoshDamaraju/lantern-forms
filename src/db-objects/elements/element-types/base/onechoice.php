...
class Onechoice extends Element_Type implements Choice_Element_Type_Interface {
	...
	protected function bootstrap() {
		$this->slug        = 'onechoice';
		$this->title       = __( 'One Choice', 'lantern-forms' );
		$this->description = __( 'A radio group element to select a single value from.', 'lantern-forms' );
		$this->icon_svg_id = 'lantern-icon-onechoice';
	...
	public function validate_field( $value, $element, $submission ) {
		...
		if ( ! empty( $value ) && ! in_array( $value, $choices, true ) ) {
			return $this->create_error( 'value_invalid_choice', __( 'You must select a valid value from the list.', 'lantern-forms' ), $value );
		}
