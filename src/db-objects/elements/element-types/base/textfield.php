...
namespace awsmug\Lantern_Forms\DB_Objects\Elements\Element_Types\Base;

use awsmug\Lantern_Forms\DB_Objects\Elements\Element_Types\Element_Type;
use awsmug\Lantern_Forms\DB_Objects\Elements\Element;
...
class Textfield extends Element_Type {
	...
	protected function bootstrap() {
		$this->slug        = 'textfield';
		$this->title       = __( 'Text', 'lantern-forms' );
		$this->description = __( 'A single text field element.', 'lantern-forms' );
		$this->icon_svg_id = 'lantern-icon-textfield';
		...
		$this->settings_fields['min_length'] = array(
			'section'       => 'settings',
			'type'          => 'number',
			'label'         => __( 'Minimum length', 'lantern-forms' ),
			'description'   => __( 'The minimum number of chars which can be typed in.', 'lantern-forms' ),
		...
		$this->settings_fields['max_length'] = array(
			'section'       => 'settings',
			'type'          => 'number',
			'label'         => __( 'Maximum length', 'lantern-forms' ),
			'description'   => __( 'The maximum number of chars which can be typed in.', 'lantern-forms' ),
			'input_classes' => array( 'small-text' ),
			'min'           => 0,
			'step'          => 1,
		);
		$this->settings_fields['input_type'] = array(
			'section'     => 'settings',
			'type'        => 'radio',
			'label'       => __( 'Input type', 'lantern-forms' ),
			'choices'     => $input_types,
			/* translators: %s: HTML input type info URL */
			'description' => sprintf( __( '* Will be validated | Not all <a href="%s" target="_blank">HTML5 input types</a> are supported by browsers!', 'lantern-forms' ), 'http://www.wufoo.com/html5/' ),
			'default'     => 'text',
		);
	...
	public function filter_json( $data, $element, $submission = null ) {
		...
		if ( ! empty( $settings['min_length'] ) && ! empty( $settings['max_length'] ) ) {
			...
			$limits_text = sprintf( __( 'Between %1$s and %2$s characters are required.', 'lantern-forms' ), number_format_i18n( $settings['min_length'] ), number_format_i18n( $settings['max_length'] ) );
		} elseif ( ! empty( $settings['min_length'] ) ) {
			/* translators: %s: minimum length */
			$limits_text = sprintf( __( 'At least %s characters are required.', 'lantern-forms' ), number_format_i18n( $settings['min_length'] ) );
		} elseif ( ! empty( $settings['max_length'] ) ) {
			/* translators: %s: maximum length */
			$limits_text = sprintf( __( 'A maximum of %s characters are allowed.', 'lantern-forms' ), number_format_i18n( $settings['max_length'] ) );
		}
	...
	public function validate_field( $value, $element, $submission ) {
		...
		if ( ! empty( $settings['required'] ) && 'no' !== $settings['required'] && empty( $value ) ) {
			return $this->create_error( Element_Type::ERROR_CODE_REQUIRED, __( 'You must enter something here.', 'lantern-forms' ), $value );
		}

		if ( ! empty( $value ) ) {
			if ( ! empty( $settings['min_length'] ) && strlen( $value ) < (int) $settings['min_length'] ) {
				return $this->create_error( 'value_too_short', __( 'The value you entered is too short.', 'lantern-forms' ), $value );
			}

			if ( ! empty( $settings['max_length'] ) && strlen( $value ) > (int) $settings['max_length'] ) {
				return $this->create_error( 'value_too_long', __( 'The value you entered is too long.', 'lantern-forms' ), $value );
			}
			...
			if ( ! empty( $settings['input_type'] ) ) {
				...
				if ( $input_type ) {
					...
					if ( ! $status ) {
						$message = ! empty( $input_type['error_message'] ) ? $input_type['error_message'] : __( 'The value you entered is invalid.', 'lantern-forms' );
	...
	protected function get_input_types() {
		$input_types = array(
			'text'           => array(
				'title'           => __( 'Standard Text', 'lantern-forms' ),
				'html_field_type' => 'text',
			),
			'password'       => array(
				'title'           => __( 'Password', 'lantern-forms' ),
				'html_field_type' => 'password',
			),
			'date'           => array(
				'title'           => __( 'Date', 'lantern-forms' ),
				'html_field_type' => 'date',
			),
			'email_address'  => array(
				'title'           => __( 'Email-Address *', 'lantern-forms' ),
				'html_field_type' => 'email',
				'callback'        => 'is_email',
				'error_message'   => __( 'The value you entered is not a valid email-address.', 'lantern-forms' ),
			),
			'color'          => array(
				'title'           => __( 'Color', 'lantern-forms' ),
				'html_field_type' => 'color',
			),
			'number'         => array(
				'title'           => __( 'Number *', 'lantern-forms' ),
				'html_field_type' => 'number',
				'pattern'         => '^[0-9]{1,}$',
				'error_message'   => __( 'The value you entered is not a valid number.', 'lantern-forms' ),
			),
			'number_decimal' => array(
				'title'           => __( 'Decimal Number *', 'lantern-forms' ),
				'html_field_type' => 'number',
				'pattern'         => '^-?([0-9])+\.?([0-9])+$',
				'error_message'   => __( 'The value you entered is not a valid decimal number.', 'lantern-forms' ),
			),
			'search'         => array(
				'title'           => __( 'Search', 'lantern-forms' ),
				'html_field_type' => 'search',
			),
			'tel'            => array(
				'title'           => __( 'Telephone', 'lantern-forms' ),
				'html_field_type' => 'tel',
			),
			'time'           => array(
				'title'           => __( 'Time', 'lantern-forms' ),
				'html_field_type' => 'time',
			),
			'url'            => array(
				'title'           => __( 'URL *', 'lantern-forms' ),
				'html_field_type' => 'url',
				'pattern'         => '\b(?:(?:https?|ftp):\/\/|www\.)[-a-z0-9+&@#\/%?=~_|!:,.;]*[-a-z0-9+&@#\/%=~_|]',
				'error_message'   => __( 'The value you entered is not a valid URL.', 'lantern-forms' ),
			),
			'week'           => array(
				'title'           => __( 'Week', 'lantern-forms' ),
