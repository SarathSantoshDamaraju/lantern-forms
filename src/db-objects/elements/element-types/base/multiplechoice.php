...
use awsmug\Lantern_Forms\DB_Objects\Elements\Element_Types\Choice_Element_Type_Trait;
...
class Multiplechoice extends Element_Type implements Choice_Element_Type_Interface {
	...
	protected function bootstrap() {
		...
		$this->title       = __( 'Multiple Choice', 'lantern-forms' );
		$this->description = __( 'A checkbox group element to select multiple values from.', 'lantern-forms' );
		$this->icon_svg_id = 'lantern-icon-multiplechoice';
	...
	public function validate_field( $value, $element, $submission ) {
		...
		if ( ! empty( $settings['required'] ) && 'no' !== $settings['required'] && empty( $value ) ) {
			return $this->create_error( Element_Type::ERROR_CODE_REQUIRED, __( 'You must select at least a single value here.', 'lantern-forms' ), $value );
		}

		if ( (int) $settings['min_choices'] > 0 && count( $value ) < (int) $settings['min_choices'] ) {
			/* translators: %s: number of minimum choices */
			return $this->create_error( 'value_too_few_choices', sprintf( _n( 'You must select at least %s value.', 'You must select at least %s values.', $settings['min_choices'], 'lantern-forms' ), $settings['min_choices'] ), $value );
		}

		if ( (int) $settings['max_choices'] > 0 && count( $value ) > (int) $settings['max_choices'] ) {
			/* translators: %s: number of maximum choices */
			return $this->create_error( 'value_too_much_choices', sprintf( _n( 'You may select a maximum of %s value.', 'You may select a maximum of %s values.', $settings['max_choices'], 'lantern-forms' ), $settings['max_choices'] ), $value );
		}
		...
		if ( ! empty( $value ) ) {
			...
			foreach ( $value as $single_value ) {
				if ( ! in_array( $single_value, $choices, true ) ) {
					return $this->create_error( 'value_invalid_choice', __( 'You must select valid values from the list.', 'lantern-forms' ), $value );
				}
			}
		}
	...
	protected function add_multiplechoices_settings_fields( $section = 'settings' ) {
		$this->settings_fields['min_choices'] = array(
			'section'       => $section,
			'type'          => 'number',
			'label'         => __( 'Minimum Choices', 'lantern-forms' ),
			'description'   => __( 'Specify the the minumum choices to select from.', 'lantern-forms' ),
		...
		$this->settings_fields['max_choices'] = array(
			'section'       => $section,
			'type'          => 'number',
			'label'         => __( 'Maximum Choices', 'lantern-forms' ),
			'description'   => __( 'Specify the the maximum choices to select from.', 'lantern-forms' ),
			'input_classes' => array( 'regular-text' ),
		);
	}
}
