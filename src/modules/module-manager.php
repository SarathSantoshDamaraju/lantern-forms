...
namespace awsmug\Lantern_Forms\Modules;

use Leaves_And_Love\Plugin_Lib\Service;
use Leaves_And_Love\Plugin_Lib\Traits\Container_Service_Trait;
use awsmug\Lantern_Forms\Modules\Access_Controls\Module as Access_Controls_Module;
use awsmug\Lantern_Forms\Modules\Actions\Module as Actions_Module;
use awsmug\Lantern_Forms\Modules\Protectors\Module as Protectors_Module;
use awsmug\Lantern_Forms\Modules\Evaluators\Module as Evaluators_Module;
use awsmug\Lantern_Forms\Modules\Form_Settings\Module as Form_Settings_Module;
use awsmug\Lantern_Forms\DB_Objects\Forms\Form_Manager;
use awsmug\Lantern_Forms\Components\Template_Tag_Handler_Manager;
...
class Module_Manager extends Service {
	...
	public function __call( $method_name, $args ) {
		if ( 'apiapi' === $method_name ) {
			return lantern()->apiapi();
		}

		if ( 'logger' === $method_name ) {
			return lantern()->logger();
		}
	...
	public function get( $slug ) {
		if ( ! isset( $this->modules[ $slug ] ) ) {
			...
			return new Error( $this->get_prefix() . 'module_not_exist', sprintf( __( 'A module with the slug %s does not exist.', 'lantern-forms' ), $slug ), __METHOD__, '1.0.0' );
		}
	...
	public function register( $slug, $module_class_name ) {
		if ( isset( $this->modules[ $slug ] ) ) {
			/* translators: %s: module slug */
			return new Error( $this->get_prefix() . 'module_already_exist', sprintf( __( 'A module with the slug %s already exists.', 'lantern-forms' ), $slug ), __METHOD__, '1.0.0' );
		}

		if ( ! class_exists( $module_class_name ) ) {
			/* translators: %s: module class name */
			return new Error( $this->get_prefix() . 'module_class_not_exist', sprintf( __( 'The class %s does not exist.', 'lantern-forms' ), $module_class_name ), __METHOD__, '1.0.0' );
		}

		if ( ! is_subclass_of( $module_class_name, Module::class ) ) {
			/* translators: %s: module class name */
			return new Error( $this->get_prefix() . 'module_class_not_allowed', sprintf( __( 'The class %s is not allowed for a module.', 'lantern-forms' ), $module_class_name ), __METHOD__, '1.0.0' );
		}
	...
	public function unregister( $slug ) {
		...
		if ( isset( $this->default_modules[ $slug ] ) ) {
			/* translators: %s: module slug */
			return new Error( $this->get_prefix() . 'module_is_default', sprintf( __( 'The default module %s cannot be unregistered.', 'lantern-forms' ), $slug ), __METHOD__, '1.0.0' );
		}
