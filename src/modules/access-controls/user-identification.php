...
class User_Identification extends Access_Control implements Submission_Modifier_Access_Control_Interface {
	...
	protected function bootstrap() {
		...
		$this->title = __( 'User Identification', 'lantern-forms' );
	}
	...
	public function can_access( $form, $submission = null ) {
		if ( $this->get_form_option( $form->id, 'prevent_edit_others_submission', true ) && $submission ) {
			$others_submission_error = new WP_Error( 'others_submission', __( 'You do not have access to this form submission.', 'lantern-forms' ) );
			...
			if ( ! empty( $submission->user_key ) ) {
				if ( filter_has_var( INPUT_COOKIE, 'lantern_identity' ) ) {
					if ( esc_attr( filter_input( INPUT_COOKIE, 'lantern_identity' ) ) !== $submission->user_key ) {
						return $others_submission_error;
					} else {
			...
			if ( ! $skip_further_checks && ( empty( $submission->user_key ) || ! isset( $_SESSION ) || empty( $_SESSION['lantern_identity'] ) || $_SESSION['lantern_identity'] !== $submission->user_key ) ) {
				return $others_submission_error;
			}
		}
		...
		if ( $this->get_form_option( $form->id, 'prevent_multiple_submissions' ) ) {
			...
			if ( in_array( 'cookie', $identification_modes, true ) && isset( $_COOKIE[ 'lantern_has_participated_form_' . $form->id ] ) && 'yes' === $_COOKIE[ 'lantern_has_participated_form_' . $form->id ] ) {
			...
			if ( is_user_logged_in() ) {
				...
				if ( in_array( 'cookie', $identification_modes, true ) && filter_has_var( INPUT_COOKIE, 'lantern_identity' ) ) {
					$identification_args['user_key'] = esc_attr( Fixes::php_filter_input( INPUT_COOKIE, 'lantern_identity' ) );
				} elseif ( isset( $_SESSION ) && ! empty( $_SESSION['lantern_identity'] ) ) {
					$identification_args['user_key'] = esc_attr( wp_unslash( $_SESSION['lantern_identity'] ) );
				}
	...
	public function set_submission_data( $submission, $form, $data ) {
		...
		if ( in_array( 'cookie', $identification_modes, true ) ) {
			if ( ! isset( $_COOKIE['lantern_identity'] ) ) {
				setcookie( 'lantern_identity', $submission->user_key, current_time( 'timestamp' ) + 3 * YEAR_IN_SECONDS );
			}
		}
	}
	...
	public function get_meta_fields() {
		...
		$meta_fields['prevent_edit_others_submission'] = array(
			'type'         => 'checkbox',
			'label'        => __( 'Prevent users from accessing someone else&apos;s submission?', 'lantern-forms' ),
			'description'  => __( 'Click the checkbox to ensure that participants cannot access other participants&apos; submissions.', 'lantern-forms' ),
			'default'      => true,
			'wrap_classes' => array( 'has-lantern-tooltip-description' ),
			'visual_label' => __( 'User per submission', 'lantern-forms' ),
		);
		$meta_fields['prevent_multiple_submissions']   = array(
			'type'         => 'checkbox',
			'label'        => __( 'Prevent multiple submissions by a single user?', 'lantern-forms' ),
			'description'  => __( 'Click the checkbox to ensure that participants may only submit this form once.', 'lantern-forms' ),
			'wrap_classes' => array( 'has-lantern-tooltip-description' ),
			'visual_label' => __( 'Single submission', 'lantern-forms' ),
		);
		$meta_fields['identification_modes']           = array(
			'type'        => 'multibox',
			'label'       => __( 'Identification Modes', 'lantern-forms' ),
			'description' => __( 'For non logged-in users, by default PHP sessions are used to identity them. You can enable further modes here to improve accuracy.', 'lantern-forms' ),
			'choices'     => array(
				'ip_address' => __( 'IP address', 'lantern-forms' ),
				'cookie'     => __( 'Cookie', 'lantern-forms' ),
			),
		);
		$meta_fields['already_submitted_message']      = array(
			'type'          => 'text',
			'label'         => __( '&#8220;Already submitted&#8221; Message', 'lantern-forms' ),
			'description'   => __( 'Enter the message to show to the user when they have already submitted this form.', 'lantern-forms' ),
	...
	protected function get_default_already_submitted_message() {
		return __( 'You already submitted this form.', 'lantern-forms' );
	}
}
