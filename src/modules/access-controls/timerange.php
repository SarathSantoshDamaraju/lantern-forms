...
class Timerange extends Access_Control {
	...
	protected function bootstrap() {
		$this->slug        = 'timerange';
		$this->title       = __( 'Timerange', 'lantern-forms' );
		$this->description = __( 'Allows you to specific a timerange in which this form is available.', 'lantern-forms' );
	}
	...
	public function get_meta_fields() {
		...
		$meta_fields['start']                  = array(
			'type'         => 'datetime',
			'label'        => __( 'Start Date', 'lantern-forms' ),
			'description'  => __( 'Select the date this form should be opened.', 'lantern-forms' ),
			'store'        => 'datetime',
			'wrap_classes' => array( 'has-lantern-tooltip-description' ),
		);
		$meta_fields['end']                    = array(
			'type'         => 'datetime',
			'label'        => __( 'End Date', 'lantern-forms' ),
			'description'  => __( 'Select the date this form should be closed.', 'lantern-forms' ),
			'store'        => 'datetime',
			'wrap_classes' => array( 'has-lantern-tooltip-description' ),
		);
		$meta_fields['not_yet_open_message']   = array(
			'type'          => 'text',
			'label'         => __( '&#8220;Not yet open&#8221; Message', 'lantern-forms' ),
			'description'   => __( 'Enter the message to show to the user in case the form is not yet open to submissions.', 'lantern-forms' ),
		...
		$meta_fields['no_longer_open_message'] = array(
			'type'          => 'text',
			'label'         => __( '&#8220;No longer open&#8221; Message', 'lantern-forms' ),
			'description'   => __( 'Enter the message to show to the user in case the form is no longer open to submissions.', 'lantern-forms' ),
	...
	protected function get_default_not_yet_open_message() {
		return __( 'This form is not yet open to submissions.', 'lantern-forms' );
	}
	...
	protected function get_default_no_longer_open_message() {
		return __( 'This form is no longer open to submissions.', 'lantern-forms' );
	}
}
