...
namespace awsmug\Lantern_Forms\Modules\Access_Controls;

use awsmug\Lantern_Forms\Modules\Assets_Submodule_Interface;
use awsmug\Lantern_Forms\DB_Objects\Forms\Form;
use awsmug\Lantern_Forms\DB_Objects\Submissions\Submission;
use awsmug\Lantern_Forms\Components\Template_Tag_Handler;
...
class Members extends Access_Control implements Assets_Submodule_Interface {
	...
	protected function bootstrap() {
		$this->slug        = 'members';
		$this->title       = __( 'Members', 'lantern-forms' );
		$this->description = __( 'Allows you to make this form available to logged-in members only.', 'lantern-forms' );
	...
	public function get_meta_fields() {
		...
		$meta_fields['allowed_roles'] = array(
			...
			'label'       => __( 'Allowed Roles', 'lantern-forms' ),
			'description' => __( 'If you select user roles here, only users with these roles are granted access to the form.', 'lantern-forms' ),
			'choices'     => $role_choices,
		);

		$meta_fields['allowed_users'] = array(
			'type'          => 'autocompletewithbutton',
			'label'         => __( 'Allowed Users', 'lantern-forms' ),
			'description'   => __( 'If you select users here, only these users are granted access to the form.', 'lantern-forms' ),
			'repeatable'    => true,
			'autocomplete'  => array(
				...
				'label_generator'               => sprintf( __( '%1$s (User ID %2$s)', 'lantern-forms' ), '%name%', '%id%' ),
			),
			'input_classes' => array( 'lantern-member-invitation-input' ),
			'button_label'  => __( 'Send Invitation', 'lantern-forms' ),
			'button_attrs'  => array(
				'class'    => 'button-link lantern-send-invitation',
				'disabled' => true,
			),
		);
		...
		$meta_fields['login_required_message'] = array(
			'type'          => 'text',
			'label'         => __( '&#8220;Login required&#8221; Message', 'lantern-forms' ),
			'description'   => __( 'Enter the message to show to the user in case they are not logged in.', 'lantern-forms' ),
		...
		$meta_fields['not_selected_message'] = array(
			'type'          => 'text',
			'label'         => __( '&#8220;Not eligible&#8221; Message', 'lantern-forms' ),
			'description'   => __( 'Enter the message to show to the user when they are logged in, but have not been selected to participate.', 'lantern-forms' ),
	...
	public function get_settings_sections() {
		...
		$settings_sections['invitation_email'] = array(
			'title'       => __( 'Invitation Email Template', 'lantern-forms' ),
			'description' => __( 'Setup mail templates for inviting a user to a form.', 'lantern-forms' ),
		);

		$settings_sections['reinvitation_email'] = array(
			'title'       => __( 'Reinvitation Email Template', 'lantern-forms' ),
			'description' => __( 'Setup mail templates for reinviting a user to a form.', 'lantern-forms' ),
		);
	...
	public function get_settings_fields() {
		...
		$settings_fields['invitation_from_name'] = array(
			...
			'label'                => __( 'From Name', 'lantern-forms' ),
			'input_classes'        => array( 'regular-text' ),
			'default'              => '{sitetitle}',
		...
		$settings_fields['invitation_from_email'] = array(
			...
			'label'                => __( 'From Email', 'lantern-forms' ),
			/* translators: %s: email address */
			'description'          => sprintf( __( 'This email address should contain the same domain like your website (e.g. %s).', 'lantern-forms' ), 'email@' . $domain ),
			'input_classes'        => array( 'regular-text' ),
			'default'              => '{adminemail}',
		...
		$settings_fields['invitation_subject'] = array(
			'section'              => 'invitation_email',
			'type'                 => 'templatetagtext',
			'label'                => __( 'Subject', 'lantern-forms' ),
			'input_classes'        => array( 'regular-text' ),
		...
		$settings_fields['invitation_message'] = array(
			...
			'label'                => __( 'Message', 'lantern-forms' ),
			'rows'                 => 12,
			'media_buttons'        => true,
			'wpautop'              => true,
	...
	public function register_assets( $assets ) {
		$assets->register_script(
			...
			array(
				...
				'in_footer'     => true,
				'localize_name' => 'lanternMemberInvitations',
				'localize_data' => array(
	...
	public function ajax_invite_member( $data ) {
		if ( empty( $data['userId'] ) || empty( $data['formId'] ) ) {
			return new WP_Error( 'missing_parameters', __( 'Missing request parameters.', 'lantern-forms' ) );
		}

		$user = get_user_by( 'id', $data['userId'] );
		if ( ! $user || ! $user->exists() ) {
			return new WP_Error( 'cannot_find_user', __( 'Could not find user.', 'lantern-forms' ) );
		}

		$form = $this->module->manager()->forms()->get( $data['formId'] );
		if ( ! $form ) {
			return new WP_Error( 'cannot_find_form', __( 'Could not find form.', 'lantern-forms' ) );
		}
		...
		if ( ! $result ) {
			if ( 'reinvitation' === $mode ) {
				...
				return new WP_Error( 'cannot_reinvite_user', sprintf( __( 'User %s could not be reinvited.', 'lantern-forms' ), $user->display_name ) );
			}

			/* translators: %s: user display name */
			return new WP_Error( 'cannot_invite_user', sprintf( __( 'User %s could not be invited.', 'lantern-forms' ), $user->display_name ) );
		}
		...
		if ( 'reinvitation' === $mode ) {
			return array(
				/* translators: %s: user display name */
				'message' => sprintf( __( 'User %s was successfully reinvited.', 'lantern-forms' ), $user->display_name ),
			);
		}

		return array(
			/* translators: %s: user display name */
			'message' => sprintf( __( 'User %s was successfully invited.', 'lantern-forms' ), $user->display_name ),
		);
	}
	...
	protected function get_default_login_required_message() {
		return __( 'You have to be logged in to participate.', 'lantern-forms' );
	}
	...
	protected function get_default_not_selected_message() {
		return __( 'You have not been selected to participate.', 'lantern-forms' );
	}
	...
	protected function get_default_invitation_subject() {
		...
		return sprintf( __( 'Invitation to the form &#8220;%s&#8221;', 'lantern-forms' ), '{formtitle}' );
	}
	...
	protected function get_default_reinvitation_subject() {
		/* translators: %s: form title */
		return sprintf( __( 'Reinvitation to the form &#8220;%s&#8221;', 'lantern-forms' ), '{formtitle}' );
	}
	...
	protected function get_default_invitation_message() {
		/* translators: %s: user display name */
		$message = sprintf( __( 'Dear %s,', 'lantern-forms' ), '{userdisplayname}' ) . "\n\n";

		/* translators: %s: form title */
		$message .= sprintf( __( 'You have been invited to participate in the form &#8220;%s&#8221; which you can find at the following URL:', 'lantern-forms' ), '{formtitle}' ) . "\n\n";
		$message .= '<a href="{formurl}">{formurl}</a>' . "\n\n";
		$message .= __( 'Thanks in advance for participating!', 'lantern-forms' ) . "\n\n";
		$message .= '{sitetitle}';
	...
	protected function get_default_reinvitation_message() {
		...
		/* translators: %s: form title */
		$message .= sprintf( __( 'As a reminder, a while ago you have been invited to participate in the form &#8220;%s&#8221; which you can find at the following URL:', 'lantern-forms' ), '{formtitle}' ) . "\n\n";
		$message .= '<a href="{formurl}">{formurl}</a>' . "\n\n";
		$message .= __( 'If you have already participated by now, please ignore this email. Thanks!', 'lantern-forms' ) . "\n\n";
		$message .= '{sitetitle}';
	...
	protected function register_template_tag_handlers() {
		$tags = array(
			'sitetitle'       => array(
				'group'       => 'global',
				'label'       => __( 'Site Title', 'lantern-forms' ),
				'description' => __( 'Inserts the site title.', 'lantern-forms' ),
				'callback'    => function() {
			...
			'sitetagline'     => array(
				'group'       => 'global',
				'label'       => __( 'Site Tagline', 'lantern-forms' ),
				'description' => __( 'Inserts the site tagline.', 'lantern-forms' ),
				'callback'    => function() {
			...
			'siteurl'         => array(
				'group'       => 'global',
				'label'       => __( 'Site URL', 'lantern-forms' ),
				'description' => __( 'Inserts the site home URL.', 'lantern-forms' ),
				'callback'    => function() {
					return home_url( '/' );
				},
			),
			'adminemail'      => array(
				'group'       => 'global',
				'label'       => __( 'Site Admin Email', 'lantern-forms' ),
				'description' => __( 'Inserts the site admin email.', 'lantern-forms' ),
				'callback'    => function() {
			...
			'userip'          => array(
				'group'       => 'global',
				'label'       => __( 'User IP', 'lantern-forms' ),
				'description' => __( 'Inserts the current user IP address.', 'lantern-forms' ),
				'callback'    => function() {
			...
			'refererurl'      => array(
				'group'       => 'global',
				'label'       => __( 'Referer URL', 'lantern-forms' ),
				'description' => __( 'Inserts the current referer URL.', 'lantern-forms' ),
				'callback'    => function() {
			...
			'formtitle'       => array(
				'group'       => 'form',
				'label'       => __( 'Form Title', 'lantern-forms' ),
				'description' => __( 'Inserts the form title.', 'lantern-forms' ),
			...
			'formurl'         => array(
				'group'       => 'form',
				'label'       => __( 'Form URL', 'lantern-forms' ),
				'description' => __( 'Inserts the URL to the form.', 'lantern-forms' ),
				'callback'    => function( $form ) {
			...
			'formediturl'     => array(
				'group'       => 'form',
				'label'       => __( 'Form Edit URL', 'lantern-forms' ),
				'description' => __( 'Inserts the edit URL for the form.', 'lantern-forms' ),
				'callback'    => function( $form ) {
			...
			'useremail'       => array(
				'group'       => 'user',
				'label'       => __( 'User Email', 'lantern-forms' ),
				'description' => __( 'Inserts the email address for the user.', 'lantern-forms' ),
			...
			'username'        => array(
				'group'       => 'user',
				'label'       => __( 'Username', 'lantern-forms' ),
				'description' => __( 'Inserts the username.', 'lantern-forms' ),
				'callback'    => function( $form, $user ) {
			...
			'userdisplayname' => array(
				'group'       => 'user',
				'label'       => __( 'User Display Name', 'lantern-forms' ),
				'description' => __( 'Inserts the full display name the user has chosen to be addressed with.', 'lantern-forms' ),
				'callback'    => function( $form, $user ) {
		...
		$groups = array(
			'global' => _x( 'Global', 'template tag group', 'lantern-forms' ),
			'form'   => _x( 'Form', 'template tag group', 'lantern-forms' ),
			'user'   => _x( 'User', 'template tag group', 'lantern-forms' ),
		);
