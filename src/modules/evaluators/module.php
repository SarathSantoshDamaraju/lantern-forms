...
namespace awsmug\Lantern_Forms\Modules\Evaluators;

use awsmug\Lantern_Forms\Modules\Module as Module_Base;
use awsmug\Lantern_Forms\Modules\Submodule_Registry_Interface;
use awsmug\Lantern_Forms\Modules\Submodule_Registry_Trait;
use awsmug\Lantern_Forms\DB_Objects\Forms\Form;
use awsmug\Lantern_Forms\DB_Objects\Submissions\Submission;
use awsmug\Lantern_Forms\DB_Objects\Submissions\Submission_Manager;
...
class Module extends Module_Base implements Submodule_Registry_Interface {
	...
	protected function bootstrap() {
		...
		$this->title       = __( 'Evaluators', 'lantern-forms' );
		$this->description = __( 'Evaluators allow evaluating form submissions, for example to generate charts and analytics.', 'lantern-forms' );
	...
	protected function maybe_show_evaluation_results( $submissions ) {
		...
		$box_id    = 'lantern-evaluations';
		$box_class = 'lantern-evaluations-box postbox' . ( in_array( $box_id, $closed, true ) ? ' closed' : '' ) . ( in_array( $box_id, $hidden, true ) ? ' hide-if-js' : '' );
		...
		<div id="<?php echo esc_attr( $box_id ); ?>" class="<?php echo esc_attr( $box_class ); ?>">
			<button type="button" class="handlediv" aria-expanded="true">
				<span class="screen-reader-text"><?php esc_html_e( 'Toggle panel: Evaluations', 'lantern-forms' ); ?></span>
				<span class="toggle-indicator" aria-hidden="true"></span>
			</button>

			<h2 class="hndle">
				<span><?php esc_html_e( 'Evaluations', 'lantern-forms' ); ?></span>
			</h2>

			<div class="inside">
				<div class="lantern-evaluations-tabs" role="tablist">
					<?php foreach ( $tabs as $slug => $data ) : ?>
						<a id="<?php echo esc_attr( 'evaluations-tab-label-' . $slug ); ?>" class="lantern-evaluations-tab" href="<?php echo esc_attr( '#evaluations-tab-' . $slug ); ?>" aria-controls="<?php echo esc_attr( 'evaluations-tab-' . $slug ); ?>" aria-selected="<?php echo $slug === $current_tab_slug ? 'true' : 'false'; ?>" role="tab">
							<?php echo esc_html( $data['title'] ); ?>
						</a>
					<?php endforeach; ?>
				</div>
				<div class="lantern-evaluations-content">
					<?php foreach ( $tabs as $slug => $data ) : ?>
						<div id="<?php echo esc_attr( 'evaluations-tab-' . $slug ); ?>" class="lantern-evaluations-tab-panel" aria-labelledby="<?php echo esc_attr( 'evaluations-tab-label-' . $slug ); ?>" aria-hidden="<?php echo $slug === $current_tab_slug ? 'false' : 'true'; ?>" role="tabpanel">
							<div class="lantern-evaluations-description-wrap">
								<p class="description"><?php echo wp_kses_data( $data['description'] ); ?></p>
							</div>
							<div id="<?php echo esc_attr( 'lantern-evaluations-results-' . $slug ); ?>" class="lantern-evaluations-results">
								<?php echo $data['content']; // WPCS: XSS OK. ?>
							</div>
							<div class="lantern-evaluations-shortcode">
								<label for="<?php echo esc_attr( 'lantern-evaluations-shortcode-' . $slug ); ?>"><?php esc_html_e( 'Charts Shortcode:', 'lantern-forms' ); ?></label>
								<input id="<?php echo esc_attr( 'lantern-evaluations-shortcode-' . $slug ); ?>" class="clipboard-field regular-text" value="<?php echo esc_attr( sprintf( "[{$this->manager()->forms()->get_prefix()}form_charts id=&quot;%d&quot; mode=&quot;%s&quot;]", $form->id, $slug ) ); ?>" readonly="readonly" />
								<button type="button" class="clipboard-button button" data-clipboard-target="#<?php echo esc_attr( 'lantern-evaluations-shortcode-' . $slug ); ?>">
									<?php $this->manager()->forms()->assets()->render_icon( 'lantern-icon-clippy', __( 'Copy to clipboard', 'lantern-forms' ) ); ?>
								</button>
							</div>
						</div>
					<?php endforeach; ?>
	...
	public function get_shortcode_content( $atts ) {
		...
		if ( empty( $atts['id'] ) ) {
			return __( 'Shortcode is missing a form ID!', 'lantern-forms' );
		}

		if ( empty( $atts['mode'] ) ) {
			return __( 'Shortcode is missing a mode!', 'lantern-forms' );
		}

		$form = lantern()->forms()->get( $atts['id'] );
		if ( ! $form ) {
			return __( 'Shortcode is using an invalid form ID!', 'lantern-forms' );
		}

		$evaluator = $this->get( $atts['mode'] );
		if ( is_wp_error( $evaluator ) ) {
			return __( 'Shortcode is using an invalid mode!', 'lantern-forms' );
		}
		...
		echo '<div id="' . esc_attr( 'lantern-evaluations-results-' . $evaluator->get_slug() ) . '" class="lantern-evaluations-results">' . "\n";
		...
		return str_replace( ' class="lantern-evaluations-subtab"', ' class="lantern-evaluations-subtab" onclick="' . esc_attr( $onclick ) . '"', ob_get_clean() );
	}
	...
	public function get_deprecated_element_chart_shortcode_content( $atts ) {
		...
		if ( ! isset( $atts['id'] ) ) {
			return __( 'Shortcode is missing an element ID!', 'lantern-forms' );
		}
		...
		if ( ! $element ) {
			return __( 'Shortcode is using an invalid element ID!', 'lantern-forms' );
		}

		$container = $element->get_container();
		if ( ! $container ) {
			return __( 'It looks like the container for this element has been removed. Please enter a different element ID into the shortcode.', 'lantern-forms' );
		}
