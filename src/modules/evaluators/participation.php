...
class Participation extends Evaluator implements Assets_Submodule_Interface {
	...
	protected function bootstrap() {
		...
		$this->title       = __( 'Participation', 'lantern-forms' );
		$this->description = __( 'Evaluates participation for a form over time.', 'lantern-forms' );
	}
	...
	public function show_results( $results, $form, $args = array() ) {
		...
		if ( $args['total'] ) {
			$tabs['total'] = array(
				'label'    => _x( 'Total', 'submission count', 'lantern-forms' ),
				'callback' => function() use ( $results, $years, $total_count, $form ) {
					...
					<p>
						<strong>
							<?php esc_html_e( 'Number of completed submissions:', 'lantern-forms' ); ?>
					...
					<script type="application/json" class="c3-chart-data">
						<?php
						echo wp_json_encode( $this->get_chart_json( $form, esc_attr( $this->slug . '-chart-total' ), $years, $year_results, __( 'Years', 'lantern-forms' ), __( 'Submission Count', 'lantern-forms' ) ) );
						?>
					</script>
					<?php
				},
				...
				'callback' => function() use ( $results, $year, $form ) {
					...
					<p>
						<strong>
							...
							echo esc_html( sprintf( __( 'Number of completed submissions in %s:', 'lantern-forms' ), $year ) );
							?>
							<?php echo absint( $total_count ); ?>
						</strong>
					</p>
					...
					<script type="application/json" class="c3-chart-data">
						<?php
						echo wp_json_encode( $this->get_chart_json( $form, esc_attr( $this->slug . '-chart-' . $year ), array_values( $months ), $month_results, __( 'Months', 'lantern-forms' ), __( 'Submission Count', 'lantern-forms' ) ) );
						?>
					</script>
					<?php
				},
	...
	public function get_meta_fields() {
		...
		$meta_fields['display_mode'] = array(
			'type'    => 'select',
			'label'   => __( 'Display Mode', 'lantern-forms' ),
			'choices' => array(
				'line'   => __( 'Line Chart', 'lantern-forms' ),
				'spline' => __( 'Spline Chart', 'lantern-forms' ),
				'step'   => __( 'Step Chart', 'lantern-forms' ),
				'bar'    => __( 'Bar Chart', 'lantern-forms' ),
			),
			'default' => 'line',
		);
		...
		$meta_fields['data_point_labels'] = array(
			'type'        => 'select',
			'label'       => _x( 'Data Point Labels', 'evaluator', 'lantern-forms' ),
			'description' => __( 'Specify whether a label should be shown with each data point and which content it should display.', 'lantern-forms' ),
			'choices'     => array(
				'none'       => _x( 'None', 'data point labels', 'lantern-forms' ),
				'value'      => _x( 'Values', 'data point labels', 'lantern-forms' ),
				'percentage' => _x( 'Percentages', 'data point labels', 'lantern-forms' ),
			),
			'default'     => 'none',
		);
