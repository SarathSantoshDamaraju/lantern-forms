...
class Element_Responses extends Evaluator implements Assets_Submodule_Interface {
	...
	protected function bootstrap() {
		...
		$this->title       = __( 'Element Responses', 'lantern-forms' );
		$this->description = __( 'Evaluates individual element responses.', 'lantern-forms' );
	}
	...
	public function show_results( $results, $form, $args = array() ) {
		...
		if ( ! empty( $tabs ) ) {
			...
			echo '<p>' . esc_html__( 'This form does not contain any evaluatable elements.', 'lantern-forms' ) . '</p>';
		}
	}
	...
	public function get_meta_fields() {
		...
		$meta_fields['display_mode'] = array(
			...
			'choices' => array(
				...
				'pie'   => __( 'Pie Chart', 'lantern-forms' ),
				'donut' => __( 'Donut Chart', 'lantern-forms' ),
			),
			'default' => 'bar',
		);
		...
		if ( 'bar' === $display_mode ) {
			...
			$data = array(
				...
				'data'   => array(
					...
					'names'   => array(
						'responseCount' => __( 'Response Count', 'lantern-forms' ),
					),
					'type'    => $display_mode,
					'labels'  => $labels,
				),
