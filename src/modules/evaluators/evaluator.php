...
use awsmug\Lantern_Forms\Modules\Submodule;
use awsmug\Lantern_Forms\Modules\Meta_Submodule_Interface;
use awsmug\Lantern_Forms\Modules\Meta_Submodule_Trait;
use awsmug\Lantern_Forms\Modules\Settings_Submodule_Interface;
use awsmug\Lantern_Forms\Modules\Settings_Submodule_Trait;
use awsmug\Lantern_Forms\DB_Objects\Forms\Form;
use awsmug\Lantern_Forms\DB_Objects\Submissions\Submission;
use awsmug\Lantern_Forms\DB_Objects\Submissions\Submission_Collection;
...
abstract class Evaluator extends Submodule implements Meta_Submodule_Interface, Settings_Submodule_Interface {
	...
	public function get_meta_fields() {
		...
		$meta_fields['enabled'] = array(
			'type'         => 'checkbox',
			'label'        => _x( 'Enable?', 'evaluator', 'lantern-forms' ),
			'visual_label' => _x( 'Status', 'evaluator', 'lantern-forms' ),
		);
	...
	protected function display_tabs( $tabs ) {
		...
		?>
		<div class="lantern-evaluations-subtabs" role="tablist">
			<?php $first = true; ?>
			<?php foreach ( $tabs as $tab_slug => $tab_data ) : ?>
				<a id="<?php echo esc_attr( $prefix . '-label-' . $tab_slug ); ?>" class="lantern-evaluations-subtab" href="<?php echo esc_attr( '#' . $prefix . '-' . $tab_slug ); ?>" aria-controls="<?php echo esc_attr( $prefix . '-' . $tab_slug ); ?>" aria-selected="<?php echo $first ? 'true' : 'false'; ?>" role="tab">
					<?php echo esc_html( $tab_data['label'] ); ?>
				</a>
				<?php $first = false; ?>
			<?php endforeach; ?>
		</div>
		<div class="lantern-evaluations-subcontent">
			<?php $first = true; ?>
			<?php foreach ( $tabs as $tab_slug => $tab_data ) : ?>
				<div id="<?php echo esc_attr( $prefix . '-' . $tab_slug ); ?>" class="lantern-evaluations-subtab-panel" aria-labelledby="<?php echo esc_attr( $prefix . '-label-' . $tab_slug ); ?>" aria-hidden="<?php echo $first ? 'false' : 'true'; ?>" role="tabpanel">
