...
namespace awsmug\Lantern_Forms\Modules\Form_Settings;
...
class Module extends Module_Base {
	...
	protected function bootstrap() {
		...
		$this->title       = __( 'Form Settings', 'lantern-forms' );
		$this->description = __( 'Form settings control the general behavior of forms.', 'lantern-forms' );
	}
	...
	protected function get_meta_tabs() {
		...
		$tabs = array(
			'labels'   => array(
				'title' => _x( 'Labels', 'form settings tab', 'lantern-forms' ),
			),
			'advanced' => array(
				'title' => _x( 'Advanced', 'form settings tab', 'lantern-forms' ),
			),
		);
	...
	protected function get_meta_fields() {
		...
		$fields = array(
			'show_container_title'  => array(
				'tab'          => 'labels',
				'type'         => 'checkbox',
				'label'        => __( 'Show page title?', 'lantern-forms' ),
				'description'  => __( 'Click the checkbox to display the title of the current page in the frontend.', 'lantern-forms' ),
				'default'      => true,
				'wrap_classes' => array( 'has-lantern-tooltip-description' ),
				'visual_label' => __( 'Page Title', 'lantern-forms' ),
			),
			'required_fields_text'  => array(
				'tab'          => 'labels',
				'type'         => 'text',
				'label'        => __( 'Required fields text', 'lantern-forms' ),
				'description'  => __( 'This text appears on top of a form and shows the user which fields are required. The value has to cotain a %s to output the correct indicator.', 'lantern-forms' ), //phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
			...
			'previous_button_label' => array(
				'tab'          => 'labels',
				'type'         => 'text',
				'label'        => __( 'Previous Button Label', 'lantern-forms' ),
				'description'  => __( 'Enter the label for the button that leads to the previous form page.', 'lantern-forms' ),
			...
			'next_button_label'     => array(
				'tab'          => 'labels',
				'type'         => 'text',
				'label'        => __( 'Next Button Label', 'lantern-forms' ),
				'description'  => __( 'Enter the label for the button that leads to the next form page.', 'lantern-forms' ),
			...
			'submit_button_label'   => array(
				'tab'          => 'labels',
				'type'         => 'text',
				'label'        => __( 'Submit Button Label', 'lantern-forms' ),
				'description'  => __( 'Enter the label for the button that submits the form.', 'lantern-forms' ),
			...
			'success_message'       => array(
				'tab'           => 'labels',
				'type'          => 'text',
				'label'         => __( 'Success Message', 'lantern-forms' ),
				'description'   => __( 'Enter a message to display when a form submission has successfully been completed.', 'lantern-forms' ),
			...
			'allow_get_params'      => array(
				'tab'         => 'advanced',
				'type'        => 'checkbox',
				'label'       => __( 'Allow GET parameters?', 'lantern-forms' ),
				/* translators: %s: GET parameter example */
				'description' => sprintf( __( 'Click the checkbox to allow initial field values to be set through GET parameters (such as %s).', 'lantern-forms' ), '<code>?lantern_input_value_ELEMENT_ID=VALUE</code>' ),
			),
		);
	...
	protected function get_default_required_fields_text() {
		return _x( 'Required fields are marked %s.', 'required fields text on top of form', 'lantern-forms' ); //phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
	}
	...
	protected function get_default_previous_button_label() {
		return _x( 'Previous Step', 'button label', 'lantern-forms' );
	}
	...
	protected function get_default_next_button_label() {
		return _x( 'Next Step', 'button label', 'lantern-forms' );
	}
	...
	protected function get_default_submit_button_label() {
		return _x( 'Submit', 'button label', 'lantern-forms' );
	}
	...
	protected function get_default_success_message() {
		return __( 'Thank you for submitting!', 'lantern-forms' );
	}
	...
	protected function filter_required_fields_text( $required_fields_text, $form_id ) {
		...
		$required_fields_text = '<span aria-hidden="true">' . sprintf( $required_fields_text, '<span class="lantern-required-indicator">*</span>' ) . '</span>';
