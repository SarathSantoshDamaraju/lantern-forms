...
class reCAPTCHA extends Protector { // @codingStandardsIgnoreLine
	...
	protected function bootstrap() {
		$this->slug  = 'recaptcha';
		$this->title = __( 'reCAPTCHA', 'lantern-forms' );
	}
	...
	public function verify_request( $data, $form, $submission = null ) {
		if ( empty( $_POST['g-recaptcha-response'] ) ) { // phpcs:ignore WordPress.Security
			return new WP_Error( 'missing_recaptcha', __( 'Missing reCAPTCHA response. Please check the reCAPTCHA checkbox to verify you are human.', 'lantern-forms' ) );
		}
		...
		try {
			...
			return new WP_Error( 'cannot_process_recaptcha', __( 'An unknown error occurred processing the reCAPTCHA response.', 'lantern-forms' ) );
		}
		...
		if ( empty( $verification['success'] ) ) {
			...
			switch ( $error_code ) {
				case 'missing-input-secret':
					return new WP_Error( 'missing_recaptcha_secret', __( 'Internal error: The reCAPTCHA secret is missing. Please contact an administrator.', 'lantern-forms' ) );
				case 'invalid-input-secret':
					return new WP_Error( 'invalid_recaptcha_secret', __( 'Internal error: The reCAPTCHA secret is invalid or malformed. Please contact an administrator.', 'lantern-forms' ) );
				case 'missing-input-response':
					return new WP_Error( 'missing_recaptcha_response', __( 'Internal error: The reCAPTCHA response is missing. Please contact an administrator.', 'lantern-forms' ) );
				case 'invalid-input-response':
					return new WP_Error( 'invalid_recaptcha_response', __( 'Internal error: The reCAPTCHA response is invalid or malformed. Please contact an administrator.', 'lantern-forms' ) );
				default:
					return new WP_Error( 'unknown_recaptcha_error', __( 'Internal error: An unknown reCAPTCHA error occurred. Please contact an administrator.', 'lantern-forms' ) );
			}
		}
	...
	public function render_output( $form ) {
		if ( ! $this->is_configured() ) {
			?>
			<div class="lantern-notice lantern-error-notice">
				<p><?php esc_html_e( 'You should actually be able to see a reCAPTCHA field here, but it is not correctly configured. Please contact an administrator.', 'lantern-forms' ); ?></p>
			</div>
			<?php
			return;
		}
		...
		?>
		<div class="lantern-recaptcha lantern-element-wrap">
			<div id="<?php echo esc_attr( 'recaptcha-placeholder-' . $form->id ); ?>" class="lantern-recaptcha-placeholder" data-form-id="<?php echo absint( $form->id ); ?>" data-sitekey="<?php echo esc_attr( $key ); ?>" data-type="<?php echo esc_attr( $type ); ?>" data-size="<?php echo esc_attr( $size ); ?>" data-theme="<?php echo esc_attr( $theme ); ?>"></div>
		</div>
		<?php
	...
	public function print_script() {
		...
		$callback_name = 'lanternReCAPTCHAWidgetsInit';
		...
		<script type="text/javascript">
			var <?php echo esc_js( $callback_name ); ?> = function() {
				var captchaTags = document.getElementsByClassName( 'lantern-recaptcha-placeholder' );
	...
	public function get_meta_fields() {
		...
		$meta_fields['enabled'] = array(
			'type'         => 'checkbox',
			'label'        => _x( 'Add captcha at the end of the form.', 'protector', 'lantern-forms' ),
			'visual_label' => _x( 'Google reCAPTCHA', 'protector', 'lantern-forms' ),
		);

		if ( ! $this->is_configured() ) {
			$meta_fields['enabled']['description'] = sprintf( __( 'reCaptcha is not configured properly. Please add a site key & secret to the <a href="%s">recaptcha settings page</a>.', 'lantern-forms' ), admin_url( 'edit.php?post_type=lantern_form&page=lantern_form_settings&tab=lantern_module_protectors' ) );
		}

		$meta_fields['type'] = array(
			'type'    => 'select',
			'label'   => _x( 'Type', 'reCAPTCHA', 'lantern-forms' ),
			'choices' => array(
				'image' => _x( 'Image', 'reCAPTCHA type', 'lantern-forms' ),
				'audio' => _x( 'Audio', 'reCAPTCHA type', 'lantern-forms' ),
			),
		);

		$meta_fields['size'] = array(
			'type'    => 'select',
			'label'   => _x( 'Size', 'reCAPTCHA', 'lantern-forms' ),
			'choices' => array(
				'normal'  => _x( 'Normal', 'reCAPTCHA size', 'lantern-forms' ),
				'compact' => _x( 'Compact', 'reCAPTCHA size', 'lantern-forms' ),
			),
		);

		$meta_fields['theme'] = array(
			'type'    => 'select',
			'label'   => _x( 'Theme', 'reCAPTCHA', 'lantern-forms' ),
			'choices' => array(
				'light' => _x( 'Light', 'reCAPTCHA theme', 'lantern-forms' ),
				'dark'  => _x( 'Dark', 'reCAPTCHA theme', 'lantern-forms' ),
			),
		);
	...
	public function get_settings_sections() {
		...
		$settings_sections['credentials'] = array(
			'title' => _x( 'Credentials', 'reCAPTCHA', 'lantern-forms' ),
		);
	...
	public function get_settings_fields() {
		...
		$settings_fields['site_key'] = array(
			'section'       => 'credentials',
			'type'          => 'text',
			'label'         => _x( 'Site Key', 'reCAPTCHA', 'lantern-forms' ),
			/* translators: %s: URL to Google reCAPTCHA console */
			'description'   => sprintf( __( 'The public site key of your website for Google reCAPTCHA. You can get one <a href="%s" target="_blank">here</a>.', 'lantern-forms' ), 'https://www.google.com/recaptcha/admin' ),
		...
		$settings_fields['secret_key'] = array(
			'section'       => 'credentials',
			'type'          => 'text',
			'label'         => _x( 'Secret', 'reCAPTCHA', 'lantern-forms' ),
			/* translators: %s: URL to Google reCAPTCHA console */
			'description'   => sprintf( __( 'The secret key of your website for Google reCAPTCHA. You can get one <a href="%s" target="_blank">here</a>.', 'lantern-forms' ), 'https://www.google.com/recaptcha/admin' ),
			'input_classes' => array( 'regular-text' ),
		);
