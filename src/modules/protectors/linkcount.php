...
class Linkcount extends Protector {
	...
	protected function bootstrap() {
		$this->slug        = 'linkcount';
		$this->title       = __( 'Link Count', 'lantern-forms' );
		$this->description = __( 'Tries to detect bots by the amount of links in the form submission data.', 'lantern-forms' );
	}
	...
	public function verify_request( $data, $form, $submission = null ) {
		...
		foreach ( $data['values'] as $element_id => $fields ) {
			foreach ( $fields as $field_slug => $value ) {
				...
				return new WP_Error( 'too_many_links', __( 'Your submission contains too many links and was therefore considered spam.', 'lantern-forms' ) );
			}
		}
	...
	public function get_meta_fields() {
		...
		$meta_fields['trigger'] = array(
			'type'         => 'number',
			'label'        => __( 'Link Count Trigger', 'lantern-forms' ),
			'description'  => __( 'Specify the maximum number of links a field is allowed to contain before it is considered spam.', 'lantern-forms' ),
			'default'      => 3,
			'min'          => 1,
			'step'         => 1,
