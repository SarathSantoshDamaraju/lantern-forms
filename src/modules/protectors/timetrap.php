...
class Timetrap extends Protector {
	...
	protected function bootstrap() {
		$this->slug        = 'timetrap';
		$this->title       = __( 'Timetrap', 'lantern-forms' );
		$this->description = __( 'Tries to detect bots by setting a minimum time that users need to fill in the form.', 'lantern-forms' );
	}
	...
	public function verify_request( $data, $form, $submission = null ) {
		...
		if ( empty( $timestamp ) ) {
			return new WP_Error( 'missing_timestamp', __( 'Internal error: Could not verify you are human. Please contact an administrator if you are.', 'lantern-forms' ) );
		}

		$trigger = $this->get_form_option( $form->id, 'trigger', 3 );

		if ( $now - $timestamp < $trigger ) {
			return new WP_Error( 'timetrap_too_quickly', __( 'You filled this form too quickly to qualify as a human. We understand you possibly are in a hurry, but you really only have to wait a few seconds to send it.', 'lantern-forms' ) );
		}
	...
	public function render_output( $form ) {
		?>
		<input type="hidden" id="lantern-timestamp" name="timestamp" value="<?php echo esc_attr( current_time( 'timestamp' ) ); ?>">
		<?php
	}
	...
	public function get_meta_fields() {
		...
		$meta_fields['trigger'] = array(
			'type'         => 'number',
			'label'        => __( 'Timetrap Trigger', 'lantern-forms' ),
			'description'  => __( 'Specify the number of minimum seconds a user needs to be on the page in order to qualify as a human.', 'lantern-forms' ),
			'default'      => 3,
			'min'          => 1,
			'step'         => 1,
