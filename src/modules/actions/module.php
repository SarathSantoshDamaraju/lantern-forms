...
class Module extends Module_Base implements Submodule_Registry_Interface {
	...
	protected function bootstrap() {
		$this->slug        = 'actions';
		$this->title       = __( 'Actions', 'lantern-forms' );
		$this->description = __( 'Actions are executed in the moment users submit their form data.', 'lantern-forms' );
		...
		foreach ( $this->submodules as $slug => $action ) {
			...
			if ( is_wp_error( $action_result ) ) {
				foreach ( $action_result->get_error_messages() as $error_message ) {
					...
					$this->manager()->logger()->warning( sprintf( _x( 'Form %1$s Submission %2$s: %3$s', 'submission process log', 'lantern-forms' ), $form->id, $submission->id, $error_message ), $this->logging_context );
				}
			}
		}
	}
