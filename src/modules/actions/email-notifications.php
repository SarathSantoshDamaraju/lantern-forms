...
namespace awsmug\Lantern_Forms\Modules\Actions;

use awsmug\Lantern_Forms\DB_Objects\Forms\Form;
use awsmug\Lantern_Forms\DB_Objects\Submissions\Submission;
use awsmug\Lantern_Forms\DB_Objects\Elements\Element_Types\Base\Textfield;
...
class Email_Notifications extends Action implements Assets_Submodule_Interface {
	...
	protected function bootstrap() {
		...
		$this->title = __( 'Email Notifications', 'lantern-forms' );
	...
	public function handle( $submission, $form ) {
		...
		foreach ( $notifications as $notification ) {
			...
			if ( ! $sent ) {
				...
				$error_message = sprintf( __( 'Email notification to %s could not be sent.', 'lantern-forms' ), $notification['to_email'] );
				if ( $this->phpmailer_error ) {
					/* translators: %s: error message */
					$error_message .= ' ' . sprintf( __( 'Original error message: %s', 'lantern-forms' ), $this->phpmailer_error->get_error_message() );
	...
	public function get_meta_fields() {
		...
		$meta_fields['notifications'] = array(
			'type'       => 'group',
			'label'      => __( 'Notifications', 'lantern-forms' ),
			'repeatable' => 8,
			'fields'     => array(
				'from_name'   => array(
				...
				'reply_email' => array(
					'type'                 => 'templatetagemail',
					'label'                => __( 'Reply Email', 'lantern-forms' ),
					'input_classes'        => array( 'regular-text' ),
					'template_tag_handler' => $this->template_tag_handler_email_only,
				),
				'to_email'    => array(
					'type'                 => 'templatetagemail',
					'label'                => __( 'To Email', 'lantern-forms' ),
					'input_classes'        => array( 'regular-text' ),
					'template_tag_handler' => $this->template_tag_handler_email_only,
				),
				'cc_email'    => array(
					'type'                 => 'templatetagemail',
					'label'                => _x( 'Cc', 'email', 'lantern-forms' ),
					'input_classes'        => array( 'regular-text' ),
					'template_tag_handler' => $this->template_tag_handler_email_only,
				),
				'bcc_email'   => array(
					'type'                 => 'templatetagemail',
					'label'                => _x( 'Bcc', 'email', 'lantern-forms' ),
					'input_classes'        => array( 'regular-text' ),
					'template_tag_handler' => $this->template_tag_handler_email_only,
				),
				'subject'     => array(
	...
	protected function register_template_tag_handlers() {
		$tags = array(
			...
			'submissionurl'      => array(
				'group'       => 'submission',
				'label'       => __( 'Submission URL', 'lantern-forms' ),
				'description' => __( 'Inserts the URL to the submission.', 'lantern-forms' ),
				'callback'    => function( $form, $submission ) {
					return add_query_arg( 'lantern_submission_id', $submission->id, get_permalink( $form->id ) );
				},
			),
			'submissionediturl'  => array(
				'group'       => 'submission',
				'label'       => __( 'Submission Edit URL', 'lantern-forms' ),
				'description' => __( 'Inserts the edit URL for the submission.', 'lantern-forms' ),
				'callback'    => function( $form, $submission ) {
					return add_query_arg(
						array(
							'post_type' => lantern()->post_types()->get_prefix() . 'form',
							'page'      => lantern()->admin_pages()->get_prefix() . 'edit_submission',
							'id'        => $submission->id,
						),
						admin_url( 'edit.php' )
					);
			...
			'submissiondatetime' => array(
				'group'       => 'submission',
				'label'       => __( 'Submission Date and Time', 'lantern-forms' ),
				'description' => __( 'Inserts the submission date and time.', 'lantern-forms' ),
				'callback'    => function( $form, $submission ) {
					...
					return sprintf( _x( '%1$s at %2$s', 'concatenating date and time', 'lantern-forms' ), $date, $time );
				},
			),
		);
		...
		$complex_tags = array(
			'allelements' => array(
				'group'       => 'submission',
				'label'       => __( 'All Element Values', 'lantern-forms' ),
				'description' => __( 'Inserts all element values from the submission.', 'lantern-forms' ),
				'callback'    => function( $form, $submission ) {
		...
		$groups = array(
			...
			'submission' => _x( 'Submission', 'template tag group', 'lantern-forms' ),
		);
	 ...
	 * @param bool $back_compat Optional. Whether to also include back-compat keys for Lantern Forms before 1.0.0-beta.9. Default false.
	...
	protected function get_dynamic_template_tags( $form, $back_compat = false ) {
		...
		foreach ( $form->get_elements() as $element ) {
			...
			$tags[ 'value_element_' . $element->id ] = array(
				...
				'label'       => sprintf( __( 'Value for &#8220;%s&#8221;', 'lantern-forms' ), $element->label ),
				/* translators: %s: element label */
				'description' => sprintf( __( 'Inserts the submission value for the element &#8220;%s&#8221;.', 'lantern-forms' ), $element->label ),
	...
	public function register_assets( $assets ) {
		...
		$assets->register_script(
			...
			array(
				'deps'          => array( 'jquery', 'lantern-template-tag-fields', 'lantern-admin-form-builder' ),
				'in_footer'     => true,
				'localize_name' => 'lanternEmailNotifications',
				'localize_data' => array(
					...
					'templateTagGroupLabel'    => _x( 'Submission', 'template tag group', 'lantern-forms' ),
					/* translators: %s: element label */
					'templateTagLabel'         => sprintf( __( 'Value for &#8220;%s&#8221;', 'lantern-forms' ), '%element_label%' ),
					/* translators: %s: element label */
					'templateTagDescription'   => sprintf( __( 'Inserts the submission value for the element &#8220;%s&#8221;.', 'lantern-forms' ), '%element_label%' ),
				),
			)
		);
	}
