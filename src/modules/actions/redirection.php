...
class Redirection extends Action {
	...
	protected function bootstrap() {
		...
		$this->title = __( 'Redirection', 'lantern-forms' );
	}
	...
	public function get_meta_fields() {
		...
		$meta_fields['type'] = array(
			'type'         => 'select',
			'label'        => __( 'Redirect Type', 'lantern-forms' ),
			'description'  => __( 'Select to which type of content to redirect the user.', 'lantern-forms' ),
			'choices'      => array(
				'redirect_none' => __( 'No Redirection', 'lantern-forms' ),
				'redirect_page' => __( 'Page Redirection', 'lantern-forms' ),
				'redirect_url'  => __( 'URL Redirection', 'lantern-forms' ),
			),
		...
		if ( $page_count > 15 ) {
			$meta_fields['page'] = array(
				'type'          => 'autocomplete',
				'label'         => __( 'Redirect Page', 'lantern-forms' ),
				'description'   => __( 'Specify the page to redirect to.', 'lantern-forms' ),
		...
		$meta_fields['url'] = array(
			'type'          => 'url',
			'label'         => __( 'Redirect URL', 'lantern-forms' ),
			'description'   => __( 'Enter the URL to redirect to.', 'lantern-forms' ),
			'placeholder'   => 'https://',
			'input_classes' => array( 'regular-text' ),
			'wrap_classes'  => array( 'has-lantern-tooltip-description' ),
			'dependencies'  => array(
