...
( function( $, wp, data ) {
	...
	$( '.lantern-send-invitation' ).each( function() {
		var $input = $( this ).prev();
		if ( $input.val() ) {
	...
	$( document ).on( 'change', '.lantern-member-invitation-input + input[type="hidden"]', function() {
		var $this   = $( this );
		var $button = $this.next( '.lantern-send-invitation' );
	...
	$( document ).on( 'click', '.lantern-send-invitation', function( e ) {
		var $button = $( this );
...
}( window.jQuery, window.wp, window.lanternMemberInvitations ) );
