...
( function( lantern, $, _, i18n ) {
	...
	_.extend( Builder.prototype, {
		 ...
		 * @since 1.0.0
		 * @access public
		 * @type {lantern.Builder.ElementTypes}
		 */
		 ...
		 * @since 1.0.0
		 * @access public
		 * @type {lantern.Builder.FormModel}
		 */
		form: undefined,
		...
		init: function() {
			...
			lantern.api.init()
				.done( _.bind( function() {
					( new lantern.api.collections.ElementTypes() ).fetch({
						data: {
							context: 'edit'
						},
						context: this,
						success: function( elementTypes ) {
							this.elementTypes = lantern.Builder.ElementTypes.fromApiCollection( elementTypes );

							if ( 'auto-draft' !== $( '#original_post_status' ).val() ) {
								( new lantern.api.models.Form({
		...
		setupInitialData: function( form ) {
			...
			if ( form ) {
				this.form = new lantern.Builder.FormModel( form );
				...
				this.form = new lantern.Builder.FormModel({});
		...
		setupViews: function() {
			...
			this.formView = new lantern.Builder.FormView( this.$el, this.form );
		...
		fail: function( message ) {
			var compiled = lantern.template( 'failure' );
	 ...
	 * @access public
	 */
	lantern.Builder.getInstance = function() {
		if ( ! builder ) {
			builder = new Builder( '#lantern-form-canvas' );
			builder.init();
		}
	...
	lantern.Builder.AddElement = {
		State: {},
		View:  {}
	};

	lantern.getFieldName = function( model, attribute ) {
		var groupSlug;

		if ( model instanceof lantern.Builder.FormModel ) {
			groupSlug = 'forms';
		} else if ( model instanceof lantern.Builder.ContainerModel ) {
			groupSlug = 'containers';
		} else if ( model instanceof lantern.Builder.ElementModel ) {
			groupSlug = 'elements';
		} else if ( model instanceof lantern.Builder.ElementChoiceModel ) {
			groupSlug = 'element_choices';
		} else if ( model instanceof lantern.Builder.ElementSettingModel ) {
		...
		return 'lantern_' + groupSlug + '[' + model.get( 'id' ) + '][' + attribute + ']';
	};

	lantern.getDeletedFieldName = function( model ) {
		var groupSlug;
		...
		return 'lantern_deleted_' + groupSlug + '[]';
	};

	lantern.askConfirmation = function( message, successCallback ) {
		var $dialog = $( '<div />' );
		...
		$dialog.dialog({
			dialogClass: 'wp-dialog lantern-dialog',
			modal: true,
			autoOpen: true,
			closeOnEscape: true,
			minHeight: 80,
	...
	lantern.Builder.i18n = i18n;

}( window.lantern, window.jQuery, window._, window.lanternBuilderI18n ) );

( function( lanternBuilder, _ ) {
	'use strict';
	...
	lanternBuilder.ElementType = ElementType;

})( window.lantern.Builder, window._ );
...
( function( lanternBuilder, _ ) {
	 ...
	 * @class
	 *
	 * @param {lantern.Builder.ElementType[]} elementTypes Registered element type objects.
	 */
	...
	_.extend( ElementTypes.prototype, {
		 ...
		 * @access public
		 *
		 * @returns {lantern.Builder.ElementType|undefined} Element type object, or undefined if not available.
		 */
		get: function( slug ) {
		 ...
		 * @access public
		 *
		 * @returns {lantern.Builder.ElementType[]} All element type objects.
		 */
		getAll: function() {
			return this.types;
		}
	 ...
	 * @access public
	 *
	 * @returns {lantern.Builder.ElementTypes} Element types object.
	 */
	ElementTypes.fromApiCollection = function( collection ) {
		...
		collection.each( function( model ) {
			...
			elementTypes.push( new lanternBuilder.ElementType( attributes ) );
		});

		return new ElementTypes( elementTypes );
	};

	lanternBuilder.ElementTypes = ElementTypes;

})( window.lantern.Builder, window._ );

( function( lantern, _, Backbone, wp ) {
	'use strict';
	...
	AddElementFrame = Frame.extend({
		...
		template:  lantern.template( 'add-element-frame' ),
		...
		createCollection: function() {
			...
			if ( ! ( collection instanceof Backbone.Collection ) ) {
				elementTypes = [];
				if ( collection instanceof lantern.Builder.ElementTypes ) {
		...
		createStates: function() {
			this.states.add([
				new lantern.Builder.AddElement.State.ElementTypeLibrary({
					title:      this.options.title,
		...
		createContent: function( content ) {
			content.view = new lantern.Builder.AddElement.View.ElementTypesBrowser({
				controller: this,
				collection: this.options.collection
			});
		},
		...
		createToolbar: function( toolbar ) {
			...
			toolbar.view = new lantern.Builder.AddElement.View.InsertElementToolbar({
				controller: this,
				items: {
					insert: {
						style:    'primary',
	...
	lantern.Builder.AddElement.View.Frame = AddElementFrame;

})( window.lantern, window._, window.Backbone, window.wp );

( function( lantern, Backbone, wp ) {
	'use strict';
	...
	ElementTypeLibrary = State.extend({
		defaults: {
			...
			title: lantern.Builder.i18n.selectElementType,
			menu: 'default',
	...
	lantern.Builder.AddElement.State.ElementTypeLibrary = ElementTypeLibrary;

})( window.lantern, window.Backbone, window.wp );
...
( function( lantern, $, _, Backbone, wp ) {
	...
	ElementTypesBrowser = View.extend({
		...
		template:  lantern.template( 'element-types-browser' ),

		events: {
			'click .lantern-element-type': 'setSelected',
			'keyup .lantern-element-type': 'setSelected'
		},
		...
		listenToSelected: function( state, selected ) {
			this.$el.find( '.lantern-element-type' ).each( function() {
				var $this = $( this );
	...
	lantern.Builder.AddElement.View.ElementTypesBrowser = ElementTypesBrowser;

})( window.lantern, window.jQuery, window._, window.Backbone, window.wp );

( function( lantern, _, wp ) {
	'use strict';
	...
	lantern.Builder.AddElement.View.InsertElementToolbar = InsertElementToolbar;

})( window.lantern, window._, window.wp );

( function( lanternBuilder, lantern, _, Backbone ) {
	'use strict';
	 ...
	 */
	lanternBuilder.BaseModel = Backbone.Model.extend({
		...
		constructor: function( attributes, options ) {
			...
			if ( ! attrs[ idAttribute ] ) {
				attrs[ idAttribute ] = lantern.generateTempId();
			}
		 ...
		 */
		isNew: function() {
			return ! this.has( this.idAttribute ) || lantern.isTempId( this.get( this.idAttribute ) );
		}
	});

})( window.lantern.Builder, window.lantern, window._, window.Backbone );
...
( function( lanternBuilder, lantern, _, Backbone ) {
	 ...
	 */
	lanternBuilder.BaseCollection = Backbone.Collection.extend({
		 ...
		 * @property {function}
		 */
		model: lanternBuilder.BaseModel,
		...
		constructor: function( models, options ) {
			...
			if ( this.urlEndpoint ) {
				this.url = lantern.api.root + lantern.api.versionString + this.urlEndpoint;
			}
...
( function( lanternBuilder, _ ) {
	 ...
	 *
	 * @class
	 * @augments lantern.Builder.BaseModel
	 */
	lanternBuilder.ContainerModel = lanternBuilder.BaseModel.extend({
		...
		constructor: function( attributes, options ) {
			...
			lanternBuilder.BaseModel.apply( this, [ attributes, options ] );

			this.elements = new lanternBuilder.ElementCollection([], {
				props: {
...
( function( lanternBuilder, _ ) {
	 ...
	 * @augments lantern.Builder.BaseModel
	 */
	lanternBuilder.ElementChoiceModel = lanternBuilder.BaseModel.extend({
...
( function( lanternBuilder, _ ) {
	 ...
	 * @augments lantern.Builder.BaseModel
	 */
	lanternBuilder.ElementModel = lanternBuilder.BaseModel.extend({
		...
		constructor: function( attributes, options ) {
			...
			this.element_choices = new lanternBuilder.ElementChoiceCollection([], {
				props: {
			...
			this.element_settings = new lanternBuilder.ElementSettingCollection([], {
				props: {
					element_id: this.get( 'id' )
				}
			});
		...
		listenTypeChanged: function( element, type ) {
			...
			element.element_type = lanternBuilder.getInstance().elementTypes.get( type );
...
( function( lanternBuilder, _ ) {
	 ...
	 * @augments lantern.Builder.BaseModel
	 */
	lanternBuilder.ElementSettingModel = lanternBuilder.BaseModel.extend({
...
( function( lanternBuilder ) {
	 ...
	 * @augments lantern.Builder.BaseModel
	 */
	lanternBuilder.FormModel = lanternBuilder.BaseModel.extend({
		...
		constructor: function( attributes, options ) {
			...
			this.containers = new lanternBuilder.ContainerCollection([], {
...
( function( lanternBuilder, _ ) {
	 ...
	 *
	 * @class
	 * @augments lantern.Builder.BaseCollection
	 */
	lanternBuilder.ContainerCollection = lanternBuilder.BaseCollection.extend({
		 ...
		 * @property {function}
		 */
		model: lanternBuilder.ContainerModel,
		...
		getDefaultAttributes: function() {
			var labelPlaceholder = lanternBuilder.i18n.defaultContainerLabel;
...
( function( lanternBuilder ) {
	 ...
	 * @augments lantern.Builder.BaseCollection
	 */
	lanternBuilder.ElementChoiceCollection = lanternBuilder.BaseCollection.extend({
		 ...
		 * @property {function}
		 */
		model: lanternBuilder.ElementChoiceModel,
...
( function( lanternBuilder ) {
	 ...
	 * @augments lantern.Builder.BaseCollection
	 */
	lanternBuilder.ElementCollection = lanternBuilder.BaseCollection.extend({
		 ...
		 * @property {function}
		 */
		model: lanternBuilder.ElementModel,
...
( function( lanternBuilder ) {
	 ...
	 * @augments lantern.Builder.BaseCollection
	 */
	lanternBuilder.ElementSettingCollection = lanternBuilder.BaseCollection.extend({
		 ...
		 * @property {function}
		 */
		model: lanternBuilder.ElementSettingModel,
...
( function( lanternBuilder ) {
	 ...
	 * @augments lantern.Builder.BaseCollection
	 */
	lanternBuilder.FormCollection = lanternBuilder.BaseCollection.extend({
		 ...
		 * @property {function}
		 */
		model: lanternBuilder.FormModel,
...
( function( lantern, $, _ ) {
	 ...
	 * @class
	 *
	 * @param {lantern.Builder.Container} container Container model.
	...
	function ContainerView( container, options ) {
		...
		this.tabTemplate = lantern.template( 'container-tab' );
		this.panelTemplate = lantern.template( 'container-panel' );
		this.footerPanelTemplate = lantern.template( 'container-footer-panel' );

		this.$tab = $( '<button />' );
		this.$tab.attr( 'type', 'button' );
		this.$tab.attr( 'id', 'container-tab-' + id );
		this.$tab.addClass( 'lantern-form-canvas-tab' );
		...
		this.$panel.addClass( 'lantern-form-canvas-panel' );
		...
		this.$footerPanel.addClass( 'lantern-form-canvas-panel' );
		...
		this.addElementFrame = new lantern.Builder.AddElement.View.Frame({
			title: lantern.Builder.i18n.selectElementType,
			buttonLabel: lantern.Builder.i18n.insertIntoContainer,
			collection: lantern.Builder.getInstance().elementTypes
		});
	}
	...
	_.extend( ContainerView.prototype, {
		...
		attach: function() {
			...
			this.$panel.find( '.drag-drop-area' ).sortable({
				handle: '.lantern-element-header',
				items: '.lantern-element',
				placeholder: 'lantern-element-placeholder',
				tolerance: 'pointer',
				start: function( e, ui ) {
		...
		listenRemove: function() {
			...
			if ( ! lantern.isTempId( id ) ) {
				$( '#lantern-deleted-wrap' ).append( '<input type="hidden" name="' + lantern.getDeletedFieldName( this.container ) + '" value="' + id + '" />' );
			}

			this.destroy();

			lantern.Builder.getInstance().trigger( 'removeContainer', [ this.container, this ] );
		},

		listenAddElement: function( element ) {
			var view = new lantern.Builder.ElementView( element, this.options );
			...
			lantern.Builder.getInstance().trigger( 'addElement', [ element, view ] );
		},

		listenChangeLabel: function( container, label ) {
			var name = lantern.escapeSelector( lantern.getFieldName( this.container, 'label' ) );

			this.$panel.find( 'input[name="' + name + '"]' ).val( label );
		},

		listenChangeSort: function( container, sort ) {
			var name = lantern.escapeSelector( lantern.getFieldName( this.container, 'sort' ) );
		...
		deleteContainer: function() {
			lantern.askConfirmation( lantern.Builder.i18n.confirmDeleteContainer, _.bind( function() {
		...
		updateElementsSorted: function( e, ui ) {
			...
			ui.item.parent().find( '.lantern-element' ).each( function( index ) {
				var $element = $( this );
				var element  = container.elements.get( $element.attr( 'id' ).replace( 'lantern-element-', '' ) );
	...
	lantern.Builder.ContainerView = ContainerView;

})( window.lantern, window.jQuery, window._ );

( function( lantern, $, _, fieldsAPI, dummyFieldManager ) {
	'use strict';
	...
	function getElementFieldId( element, field ) {
		return 'lantern_element_' + element.get( 'id' ) + '_' + field;
	}
	...
	function parseFields( fields, element ) {
		...
		_.each( fields, function( field ) {
			...
			if ( field.is_choices ) {
				...
				parsedField.itemInitial.label        = lantern.Builder.i18n.elementChoiceLabel.replace( '%s', '%indexPlus1%' );
				parsedField.itemInitial.name         = 'lantern_element_choices[' + tempId + '_%index%][value]';
				...
				_.each( elementChoices, function( elementChoice, index ) {
					...
					newItem.name = lantern.getFieldName( elementChoice, 'value' );
				...
				if ( field.is_label ) {
					...
					parsedField.name = lantern.getFieldName( element, 'label' );
					...
					parsedField.name = lantern.getFieldName( elementSetting, 'value' );
	 ...
	 * @class
	 *
	 * @param {lantern.Builder.Element} element Element model.
	 * @param {object}                options View options.
	 */
	function ElementView( element, options ) {
		...
		this.wrapTemplate = lantern.template( 'element' );
		this.sectionTabTemplate = lantern.template( 'element-section-tab' );
		this.sectionPanelTemplate = lantern.template( 'element-section-panel' );
		this.fieldTemplate = lantern.template( 'element-field' );

		this.$wrap = $( '<div />' );
		this.$wrap.attr( 'id', 'lantern-element-' + id );
		this.$wrap.addClass( 'lantern-element' );
	}
	...
	_.extend( ElementView.prototype, {
		...
		initializeSections: function() {
			var $sectionTabsWrap   = this.$wrap.find( '.lantern-element-content-tabs' );
			var $sectionPanelsWrap = this.$wrap.find( '.lantern-element-content-panels' );
		...
		initializeFields: function() {
			this.fieldManager = new fieldsAPI.FieldManager( parseFields( this.element.element_type.getFields(), this.element ), {
				instanceId: 'lantern_element_' + this.element.get( 'id' )
			});
			this.fieldViews = [];
			...
			_.each( this.fieldManager.models, _.bind( function( field ) {
				...
				var $sectionFieldsWrap = this.$wrap.find( '#element-panel-' + this.element.get( 'id' ) + '-' + field.get( 'section' ) + ' > .lantern-element-fields' );
				var view;
		...
		deinitializeFields: function() {
			...
			this.$wrap.find( '.lantern-element-fields' ).each( function() {
				$( this ).empty();
			});
		...
		attach: function() {
			...
			_.each( this.fieldViews, _.bind( function( fieldView ) {
				...
				} else if ( 'lanternchoices' === fieldView.model.get( 'slug' ) ) {
			...
			this.$wrap.on( 'click', '.lantern-element-header', _.bind( this.toggleActive, this ) );
			this.$wrap.on( 'click', '.lantern-element-expand-button', _.bind( this.toggleActive, this ) );
			this.$wrap.on( 'click', '.delete-element-button', _.bind( this.deleteElement, this ) );
			this.$wrap.on( 'click', '.lantern-element-content-tab', _.bind( this.changeActiveSection, this ) );
			this.$wrap.on( 'keyup change', 'input[type="text"]#' + getElementFieldId( this.element, 'label' ), _.bind( this.updateLabel, this ) );
			this.$wrap.find( '.plugin-lib-repeatable-lanternchoices-wrap' ).each( function() {
				$( this ).sortable({
					containment: 'parent',
					handle: '.lantern-element-choice-move',
					items: '.plugin-lib-repeatable-item',
					placeholder: 'lantern-element-choice-placeholder',
					tolerance: 'pointer',
		...
		detach: function() {
			...
			this.$wrap.off( 'click', '.lantern-element-content-tab', _.bind( this.changeActiveSection, this ) );
			this.$wrap.off( 'click', '.delete-element-button', _.bind( this.deleteElement, this ) );
			this.$wrap.off( 'click', '.lantern-element-expand-button', _.bind( this.toggleActive, this ) );
			this.$wrap.off( 'click', '.lantern-element-header', _.bind( this.toggleActive, this ) );
		...
		listenRemove: function() {
			...
			if ( ! lantern.isTempId( id ) ) {
				$( '#lantern-deleted-wrap' ).append( '<input type="hidden" name="' + lantern.getDeletedFieldName( this.element ) + '" value="' + id + '" />' );
			}

			this.destroy();

			lantern.Builder.getInstance().trigger( 'removeElement', [ this.element, this ] );
		},

		listenChangeLabel: function( element, label ) {
			var name          = lantern.escapeSelector( lantern.getFieldName( this.element, 'label' ) );
			...
			if ( elementHeader ) {
				...
				if ( elementHeader.length ) {
					this.$wrap.find( '.lantern-element-header-title' ).text( elementHeader );
					return;
				}
			}

			this.$wrap.find( '.lantern-element-header-title' ).text( this.element.element_type.getTitle() );
		},

		listenChangeType: function( element, type ) {
			var name = lantern.escapeSelector( lantern.getFieldName( this.element, 'type' ) );

			this.$wrap.find( 'input[name="' + name + '"]' ).val( type );
		},

		listenChangeSort: function( element, sort ) {
			var name = lantern.escapeSelector( lantern.getFieldName( this.element, 'sort' ) );
		...
		listenChangeActiveSection: function( element, activeSection ) {
			var $button = this.$wrap.find( '.lantern-element-content-tab[data-slug="' + activeSection + '"]' );

			this.$wrap.find( '.lantern-element-content-tab' ).attr( 'aria-selected', 'false' );
			this.$wrap.find( '.lantern-element-content-panel' ).attr( 'aria-hidden', 'true' );
		...
		listenChangeActive: function( props, active ) {
			if ( active.includes( this.element.get( 'id' ) ) ) {
				this.$wrap.find( '.lantern-element-expand-button' ).attr( 'aria-expanded', 'true' ).find( '.screen-reader-text' ).text( lantern.Builder.i18n.hideContent );
				this.$wrap.find( '.lantern-element-content' ).addClass( 'is-expanded' );
			} else {
				this.$wrap.find( '.lantern-element-expand-button' ).attr( 'aria-expanded', 'false' ).find( '.screen-reader-text' ).text( lantern.Builder.i18n.showContent );
				this.$wrap.find( '.lantern-element-content' ).removeClass( 'is-expanded' );
			}
		...
		listenAddElementChoiceField: function( model, addedChoiceItem ) {
			var elementChoiceId = addedChoiceItem.name.replace( 'lantern_element_choices[', '' ).replace( '][value]', '' );
			var $elementChoicesRepeatableWrap = $( '#lantern_element_' + this.element.get( 'id' ) + '_choices_' + addedChoiceItem.field + '-repeatable-wrap' );
		...
		listenRemoveElementChoiceField: function( model, removedChoiceItem ) {
			var elementChoiceId = removedChoiceItem.name.replace( 'lantern_element_choices[', '' ).replace( '][value]', '' );
			var elementChoice = this.element.element_choices.get( elementChoiceId );

			this.element.element_choices.remove( elementChoiceId );

			if ( ! lantern.isTempId( elementChoiceId ) ) {
				$( '#lantern-deleted-wrap' ).append( '<input type="hidden" name="' + lantern.getDeletedFieldName( elementChoice ) + '" value="' + elementChoiceId + '" />' );
			}
		},

		listenChangeElementChoiceFieldValue: function( model, changedChoiceItem, value ) {
			var elementChoiceId = changedChoiceItem.name.replace( 'lantern_element_choices[', '' ).replace( '][value]', '' );
			var elementChoice = this.element.element_choices.get( elementChoiceId );
		...
		deleteElement: function() {
			lantern.askConfirmation( lantern.Builder.i18n.confirmDeleteElement, _.bind( function() {
		...
		updateElementChoicesSorted: function( e, ui ) {
			...
			ui.item.parent().find( '.plugin-lib-repeatable-item' ).each( function( index ) {
				var elementChoiceId = $( this ).find( 'input[type="text"]' ).attr( 'name' ).replace( 'lantern_element_choices[', '' ).replace( '][value]', '' );
				...
				$( this ).find( 'input[name="' + lantern.escapeSelector( 'lantern_element_choices[' + elementChoiceId + '][sort]' ) + '"]' ).val( index );
			});
	...
	lantern.Builder.ElementView = ElementView;

})( window.lantern, window.jQuery, window._, window.pluginLibFieldsAPI, window.pluginLibFieldsAPIData.field_managers.lantern_dummy_1 );
...
( function( lantern, $, _ ) {
	 ...
	 * @param {lantern.Builder.Form} form    Form model.
	 * @param {object}             options View options.
	 */
	function FormView( $canvas, form, options ) {
		...
		this.canvasTemplate = lantern.template( 'form-canvas' );
	...
	_.extend( FormView.prototype, {
		render: function() {
			...
			$deletedWrap = $( '<div />' );
			$deletedWrap.attr( 'id', 'lantern-deleted-wrap' );
		...
		listenAddContainer: function( container ) {
			var view = new lantern.Builder.ContainerView( container, this.options );

			view.$tab.insertBefore( this.$addButton );
			view.$panel.insertBefore( this.$addPanel );
			this.$canvas.find( '.lantern-form-canvas-footer' ).append( view.$footerPanel );

			view.render();

			lantern.Builder.getInstance().trigger( 'addContainer', [ container, view ] );
		},
	...
	lantern.Builder.FormView = FormView;
...
( function( $ ) {
	...
	$( '.lantern-metabox-tab' ).on( 'click', function( e ) {
		var $this = $( this );
		var $all  = $this.parent().children( '.lantern-metabox-tab' );
